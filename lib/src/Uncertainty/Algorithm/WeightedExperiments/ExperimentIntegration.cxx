...
Point ExperimentIntegration::integrate(const Function & g) const
{
  ...
  {
    Point functionSampleMarginal(outputSample.getMarginal(i).asPoint());
...
Point ExperimentIntegration::computeL2Norm(const Function & g) const
{
  const UnsignedInteger outputDimension = g.getOutputDimension();
  Point weights(0);
  Sample inputSample(weightedExperiment_.generateWithWeights(weights));
  ...
  SymbolicFunction squareFunction(inputVariables, formula);
  Function integrand(ComposedFunction(squareFunction, g));
  Point functionSquaredNorm(integrate(integrand));
