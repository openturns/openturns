...
UnsignedInteger QuantileConfidence::computeUnilateralRank(const UnsignedInteger size, const Bool tail) const
{
  ...
  const Scalar p = tail ? std::pow(1.0 - alpha_, 1.0 * size) : 1.0 - std::pow(alpha_, 1.0 * size);

  if (tail && (p > 1.0 - beta_))
    throw InvalidArgumentException(HERE) << "Cannot compute rank as parameters do not satisfy (1 - alpha)^n >= 1 - beta";
  if (!tail && (p < beta_))
...
{
public:
  WilksEvaluation(const Scalar alpha,
...
UnsignedInteger QuantileConfidence::computeUnilateralMinimumSampleSize(const UnsignedInteger rank, const Bool tail) const
{
  ...
  Scalar nApprox = 0.0;
  const Function wilksConstraint(WilksEvaluation(alpha_, rank, tail).clone());
  if (rank == 0)
  {
    if (tail)
