...
OMPExpansion::OMPExpansion(const Sample & inputSample,
			     ...
			     const String & methodName,
			     const FittingAlgorithm & fitting)
  : OMPExpansion(inputSample, Point(inputSample.getSize(), 1.0 / inputSample.getSize()), outputSample, distribution, basis, basisSize, activeFunctions, methodName, fitting)
{
  // Nothing to do
}
...
void OMPExpansion::run()
{
  ...
  // Build a candidate lookup from activeFunctions_ for efficient membership check
  ...
  {
    // The constant function is always the first selected function
    ...
    // The initial values are the centered marginal output sample
    ...
    // Precompute the weighted residual W * r where r = y - Phi * beta
	...
	// No valid candidate found, stop the loop
	...
	// Update the weighted residual: W * r = W * (y - Phi * beta)
	// weightedDesign = sqrt(W) * Phi, so sqrt(W) * weightedDesign * beta = W * Phi * beta
