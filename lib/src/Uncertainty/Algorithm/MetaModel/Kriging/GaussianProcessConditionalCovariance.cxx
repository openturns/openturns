...
Graph GaussianProcessConditionalCovariance::draw(const Sample & xi, const Scalar confidenceLevel) const
{
  ...
  const Scalar quantileAlpha = DistFunc::qNormal(0.5 * (1.0 + confidenceLevel));
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    const Scalar scaledStandardDeviation = quantileAlpha * std::sqrt(conditionalVariance(i, 0));
