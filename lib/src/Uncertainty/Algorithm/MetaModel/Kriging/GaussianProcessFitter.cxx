...
GaussianProcessFitter::GaussianProcessFitter(const Sample & inputSample,
  ...
  , optimizeParameters_(ResourceMap::GetAsBool("GaussianProcessFitter-OptimizeParameters"))
  , analyticalAmplitude_(false)
  , lastReducedLogLikelihood_(SpecFunc::LowestScalar)
{
...
void GaussianProcessFitter::run()
{
  ...
  result_ = GaussianProcessFitterResult(inputSample_, outputSample_, metaModel, residuals, relativeErrors, F_, basis_, beta_, reducedCovarianceModelCopy, rho_, optimalLogLikelihood, method_);
...
Point GaussianProcessFitter::computeReducedLogLikelihood(const Point & parameters) const
{
...
Scalar GaussianProcessFitter::computeLapackLogDeterminantCholesky() const
{
  ...
  Scalar maxEV = -1.0;
  const Scalar startingScaling = ResourceMap::GetAsScalar("GaussianProcessFitter-StartingScaling");
  const Scalar maximalScaling = ResourceMap::GetAsScalar("GaussianProcessFitter-MaximalScaling");
  ...
  if (maxEV > 0.0 && scaling >= maximalScaling * maxEV)
    throw InvalidArgumentException(HERE) << "In GaussianProcessFitter::computeLapackLogDeterminantCholesky, could not compute the Cholesky factor."
...
Scalar GaussianProcessFitter::computeHMatLogDeterminantCholesky() const
{
...
Point GaussianProcessFitter::getRho() const
{
  return rho_;
}
...
UnsignedInteger GaussianProcessFitter::getMethod() const
{
  return method_;
}
...
void GaussianProcessFitter::setMethod(const UnsignedInteger method)
{
