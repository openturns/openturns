  ...
  , gamma_(gamma)
{
  ...
  if (gamma.getSize() != inputSample.getSize()) throw InvalidArgumentException(HERE) << "In GaussianProcessGradient::GaussianProcessGradient, error: the number of covariance coefficients=" << gamma.getSize() << " is different from the input sample size=" << inputSample.getSize();
...
Matrix GaussianProcessGradient::gradient(const Point & inP) const
{
  ...
  for (UnsignedInteger i = 0; i < trainingSize; ++ i)
  {
    ...
    for (UnsignedInteger j = 0; j < getInputDimension(); ++ j)
    {
      ...
      const Matrix gradient_i_j_row(gradient_i.getRow(j));
      const MatrixImplementation & grad_row_impl(*gradient_i_j_row.getImplementation());
      const Matrix gradient_i_j(dimension, dimension, grad_row_impl.begin(), grad_row_impl.end());
