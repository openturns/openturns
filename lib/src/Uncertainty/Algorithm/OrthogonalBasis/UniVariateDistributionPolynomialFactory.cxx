//                                               -*- C++ -*-
/**
 *  @brief UniVariateDistributionPolynomialFactory builds orthonormal polynomial families for a given univariate distribution
 *
  ...
  , orthonormalizationAlgorithm_(AdaptiveStieltjesAlgorithm(measure.getStandardRepresentative()))
{
  ...
  {
    a_ = specificFamily_.getA();
    b_ = specificFamily_.getB();
  }
  initializeCache();
}
  ...
  , orthonormalizationAlgorithm_(orthonormalizationAlgorithm)
{
  ...
  if (hasSpecificFamily_)
  {
    // The recurrence coefficients are those of the specific family, defined
    // with respect to the standard representative: use its affine transformation
    a_ = specificFamily_.getA();
    b_ = specificFamily_.getB();
  }
  else
  {
    // The recurrence coefficients are computed by the user-provided algorithm
    // with respect to its own measure: no affine transformation must be applied here
    a_ = 1.0;
    b_ = 0.0;
  }
  initializeCache();
