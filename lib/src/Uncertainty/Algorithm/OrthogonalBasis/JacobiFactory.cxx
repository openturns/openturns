...
{
  // alpha_ and beta_ are Beta shape parameters (A, B).
  // The Jacobi weight exponents are a = A-1, b = B-1.
  // The old code stored a, b directly and used formulas with (alpha_+1), (beta_+1).
  // Here we use A=alpha_, B=beta_ directly in formulas derived by substituting
  // a = alpha_-1, b = beta_-1 into the old formulas.
  ...
  const Scalar nAlphaP1 = n + alpha_;
  const Scalar nBetaP1 = n + beta_;
  const Scalar twoNAlphaBetaP2 = nAlphaP1 + nBetaP1;
  const Scalar factor1 = (twoNAlphaBetaP2 + 1.0) / ((n + 1.0) * nAlphaP1 * nBetaP1 * (nAlphaP1 + beta_ - 1.0));
  ...
  if (n == 1)
  {
    ...
    {
      recurrenceCoefficients[2] = (1.5 * epsilon / (beta_ - 2.0) + (beta_ - 1.0) * (3.0 - 3.125 * epsilon)) * sqrt(2.0 * beta_ / ((beta_ - 1.0) * (beta_ - 2.0) * (beta_ + 1.0)));
      return recurrenceCoefficients;
    }
  }
  recurrenceCoefficients[2] = -twoNAlphaBetaP2 / (twoNAlphaBetaP2 - 2.0) * sqrt((nAlphaP1 - 1.0) * (nBetaP1 - 1.0) * (nAlphaP1 + beta_ - 2.0) * n * factor1 / (twoNAlphaBetaP2 - 3.0));
  return recurrenceCoefficients;
}
