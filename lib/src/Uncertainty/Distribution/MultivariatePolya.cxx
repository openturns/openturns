...
/* Parameters constructor with total concentration */
MultivariatePolya::MultivariatePolya(const UnsignedInteger n,
                                     const Point & alpha,
                                     const Scalar c,
                                     const Scalar total)
...
Bool MultivariatePolya::operator ==(const MultivariatePolya & other) const
{
  ...
  return hasEqualBase(other) && (n_ == other.n_) && (alpha_ == other.alpha_) && (c_ == other.c_) && (q_ == other.q_);
}
...
{
  OSS oss;
  oss << getClassName() << "(n = " << n_ << ", alpha = " << alpha_.__str__() << ", c = " << c_ << ", total = " << q_ << ")";
  return oss;
}
...
Point MultivariatePolya::getRealization() const
{
  const UnsignedInteger dimension = getDimension();
  Scalar sumAlpha = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i) sumAlpha += alpha_[i];
  const Scalar rest = q_ - sumAlpha;
  // In the degenerate multinomial case the probabilities are fixed, mimicking Multinomial
  if (c_ == 0.0)
  {
    const Point p(alpha_ / q_);
    return Multinomial(n_, p).getRealization();
  }
  // Classical case with empty rest: Dirichlet for the tracked categories only
  if (rest == 0.0)
  {
  ...
  // Defective case: Dirichlet for the full (d+1) categories including the rest
  Point pTilde(dimension + 1);
  Scalar gammaSum = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i)
  {
    const Scalar gammaI = DistFunc::rGamma(u_[i]);
  ...
  const Scalar uRest = rest / c_;
  const Scalar gammaRest = DistFunc::rGamma(uRest);
  gammaSum += gammaRest;
  pTilde[dimension] = gammaRest;
  for (UnsignedInteger i = 0; i < dimension + 1; ++i) pTilde[i] /= gammaSum;
  const Point full(drawMultinomial(n_, pTilde));
  Point realization(dimension);
  for (UnsignedInteger i = 0; i < dimension; ++i) realization[i] = full[i];
  return realization;
}
...
Sample MultivariatePolya::getSample(const UnsignedInteger size) const
{
  ...
  for (UnsignedInteger i = 0; i < dimension; ++i) sumAlpha += alpha_[i];
  const Scalar rest = q_ - sumAlpha;
  const Point pFixed((c_ == 0.0) ? (alpha_ / q_) : Point(dimension));
  Point pTilde(dimension + 1);
  for (UnsignedInteger k = 0; k < size; ++k)
  {
    if (c_ > 0.0)
    {
      if (rest == 0.0)
      {
        // Classical case: Dirichlet for the tracked categories only
        Point pTildeClassical(dimension);
        Scalar gammaSum = 0.0;
        for (UnsignedInteger i = 0; i < dimension; ++i)
        {
          const Scalar gammaI = DistFunc::rGamma(u_[i]);
          gammaSum += gammaI;
          pTildeClassical[i] = gammaI;
        }
        for (UnsignedInteger i = 0; i < dimension; ++i) pTildeClassical[i] /= gammaSum;
        const Point realization(drawMultinomial(n_, pTildeClassical));
        for (UnsignedInteger i = 0; i < dimension; ++i) sample(k, i) = realization[i];
      }
      else
      {
        // Defective case: Dirichlet-Multinomial including the rest, mimicking the realization
        Scalar gammaSum = 0.0;
        for (UnsignedInteger i = 0; i < dimension; ++i)
        {
          const Scalar gammaI = DistFunc::rGamma(u_[i]);
    ...
    {
      const Point realization(Multinomial(n_, pFixed).getRealization());
      for (UnsignedInteger i = 0; i < dimension; ++i) sample(k, i) = realization[i];
    }
  }
...
Scalar MultivariatePolya::computePDF(const Point & point) const
{
  ...
  // The sum of the components must be less or equal to n_, with equality when the rest is empty
  if (sumX > n_) return 0.0;
  const Scalar rest = q_ - sumAlpha;
  const UnsignedInteger restDraws = n_ - sumX;
  if ((rest == 0.0) && (sumX != n_)) return 0.0;
  Scalar logPDF = 0.0;
  if (c_ > 0.0)
  {
    // P(X = x) = n! / Po(Q, n; c) * prod_j Po(alpha_j, x_j; c) / x_j! * Po(rest, n - s; c) / (n - s)!
    // with Po(x, m; c) = c^m Gamma(x/c + m) / Gamma(x/c), so that
    // log P(X = x) = lgamma(n+1) - sum_j lgamma(x_j+1) - lgamma(n-s+1)
    //     + sum_j [lgamma(u_j + x_j) - lgamma(u_j)] + [lgamma(uRest + n-s) - lgamma(uRest)]
    //     - [lgamma(U + n) - lgamma(U)], with U = Q / c
    logPDF = SpecFunc::LogGamma(1.0 * n_ + 1.0) - SpecFunc::LogGamma(1.0 * restDraws + 1.0);
    for (UnsignedInteger i = 0; i < dimension; ++i)
    {
    ...
    {
      const Scalar uRest = rest / c_;
      const Scalar restDrawsP = 1.0 * restDraws;
      logPDF += SpecFunc::LogGamma(uRest + restDrawsP) - SpecFunc::LogGamma(uRest);
    }
  ...
  else
  {
    ...
    return computeDegenerateDistribution().computePDF(point);
  }
...
Scalar MultivariatePolya::computeCDF(const Point & point) const
{
  ...
  // In the degenerate multinomial case, rely on the multinomial implementation, mimicking Multinomial
  if (c_ == 0.0) return computeDegenerateDistribution().computeCDF(point);
  Scalar sumAlpha = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i) sumAlpha += alpha_[i];
  const Scalar rest = q_ - sumAlpha;
  // Defective case (rest > 0): delegate to the full (d+1) classical distribution, mimicking Multinomial
  // The 1D defective case reduces to the beta-binomial distribution, seen as the first marginal of the full 2D
  if (rest > 0.0)
  {
    Point fullAlpha(alpha_);
    fullAlpha.add(rest);
    const MultivariatePolya full(n_, fullAlpha, c_);
    Point fullPoint(point);
    fullPoint.add(1.0 * n_);
    return full.computeCDF(fullPoint);
  }
  // Classical case with empty rest: the single component is equal to n_
...
Scalar MultivariatePolya::computeProbability(const Interval & interval) const
{
  ...
  Scalar sumAlphaProb = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i) sumAlphaProb += alpha_[i];
  // Deal with the defective case first, mimicking Multinomial
  if (sumAlphaProb < q_)
  {
    const Scalar restProb = q_ - sumAlphaProb;
    Point fullAlpha(alpha_);
    fullAlpha.add(restProb);
    Point lower(interval.getLowerBound());
    Point upper(interval.getUpperBound());
    lower.add(0.0);
    upper.add(1.0 * n_);
    return MultivariatePolya(n_, fullAlpha, c_).computeProbability(Interval(lower, upper));
  }
  // Early exit for 1D classical case, the single component is equal to n_
  if (dimension == 1)
  {
...
Distribution MultivariatePolya::getMarginal(const UnsignedInteger i) const
{
  ...
  // General case, mimicking Multinomial: the 1D marginal is a 1D defective Polya (beta-binomial)
...
Distribution MultivariatePolya::getMarginal(const Indices & indices) const
{
  ...
  // General case, mimicking Multinomial: the marginal of a subset is a defective Polya
  // with the same number of draws, reinforcement and total concentration
  MultivariatePolya marginal(n_, alpha_.select(indices), c_, q_);
...
Sample MultivariatePolya::getSupport() const
{
  ...
  const UnsignedInteger start = (rest == 0.0 ? enumerate.getStrataCumulatedCardinal(n_ - 1) : 0);
...
MultivariatePolya::PointWithDescriptionCollection MultivariatePolya::getParametersCollection() const
{
  ...
  if (dimension > 1)
  {
    PointWithDescription point(dimension + 3);
    Description description(dimension + 3);
    point[0] = n_;
    description[0] = "n";
    point[1] = c_;
    description[1] = "c";
...
{
  const UnsignedInteger dimension = getDimension();
  if (parameter.getDimension() != (dimension + 3))
    throw InvalidArgumentException(HERE) << "MultivariatePolya expected " << (dimension + 3) << " parameters, got " << parameter.getDimension();
  ...
  std::copy(parameter.begin() + 3, parameter.end(), alpha.begin());
  Scalar sum = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i) sum += alpha[i];
  // Mimicking Multinomial renormalization: when the total is smaller than the sum, use the sum (classical case)
  Scalar total = parameter[2];
...
void MultivariatePolya::setAlpha(const Point & alpha)
{
  ...
  // Mimicking Multinomial: the tracked concentrations may sum to less than the total,
  // the remainder being an implicit category. For backward compatibility the total defaults
  // to the sum when it is still unset or smaller than the sum.
  if (q_ < sum) q_ = sum;
  if (sum > q_) throw InvalidArgumentException(HERE) << "the sum of alpha = " << sum << " must be less or equal to the total = " << q_;
  alpha_ = alpha;
...
/* Total concentration accessor */
void MultivariatePolya::setTotal(const Scalar total)
{
  Scalar sum = 0.0;
  for (UnsignedInteger i = 0; i < alpha_.getDimension(); ++i) sum += alpha_[i];
  if (!(total >= sum)) throw InvalidArgumentException(HERE) << "the total = " << total << " must be greater or equal to the sum of alpha = " << sum;
  if (total != q_)
  {
    q_ = total;
    updateSpectral();
...
/* Total concentration accessor */
Scalar MultivariatePolya::getTotal() const
{
  return q_;
}
                                                ...
                                                const Bool) const
{
  ...
  Scalar sumAlpha = 0.0;
  for (UnsignedInteger i = 0; i < alpha_.getDimension(); ++i) sumAlpha += alpha_[i];
  const Scalar rest = q_ - sumAlpha;
  // Classical case with empty rest: the single component is equal to n_
  if (rest == 0.0) return 1.0 * n_;
  // Defective 1D case (beta-binomial): invert the CDF by dichotomy, mimicking the 1D reduction
  if (prob == 0.0) return 0.0;
  if (prob == 1.0) return 1.0 * n_;
  for (UnsignedInteger k = 0; k <= n_; ++k)
  {
    if (computeCDF(Point(1, 1.0 * k)) >= prob) return 1.0 * k;
  }
  return 1.0 * n_;
}
