                           ...
                           const Bool tail)
{
  ...
  if (nu > 1e3)
  {
    const NumericalScalar inu(1.0 / nu);
                                     ...
                                     const Bool tail)
{
  ...
  /*******************************/
  NumericalScalar lowerBound(0.0);
  NumericalScalar upperBound(0.0);
  NumericalScalar mode(0.0);
  // const = -(log(2)+log(2*pi)/2)
  const NumericalScalar const1(-1.6120857137646180667900353000732139);
  // logRelTolBound = log(eps) eps = 2.220446049250313e-16
  const NumericalScalar logRelTolBound(-3.604365338911715e+01);
  // zUpperBound > -norminv(eps(0)) =  3.847234634276879e+01
  const NumericalScalar zUpperBound(38.5);
  // tLower = log(1/(1-eps)) ~ eps
  const NumericalScalar tUpper(7.208730677823431e+01); // tUpper = log(1/eps^2)
  const NumericalScalar tLower(4.930380657631324e-32); // tLower = log(1/(1-eps^2)) ~ eps^2
  const NumericalScalar nuMinus2(std::max(1.0, (nu - 2.0)));
