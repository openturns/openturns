...
void MultivariateUniform::computeRange()
{
  ...
  for (UnsignedInteger i = 0; i < dimension; ++ i)
    logNormalizationFactor_ -= std::log(b_[i] - a_[i]);
}
...
Scalar MultivariateUniform::computePDF(const Point & point) const
{
  ...
  return std::exp(logNormalizationFactor_);
}
...
Scalar MultivariateUniform::computeCDF(const Point & point) const
{
  ...
  for (UnsignedInteger i = 0; i < dimension; ++i)
  {
    const Scalar x = std::min(point[i], b_[i]);
    if (x <= a_[i]) return 0.0;
    logCDF += std::log(x - a_[i]);
  }
  return std::exp(logCDF + logNormalizationFactor_);
}
...
Scalar MultivariateUniform::computeSurvivalFunction(const Point & point) const
{
  const UnsignedInteger dimension = getDimension();
  if (point.getDimension() != dimension) throw InvalidArgumentException(HERE) << "Error: the given point must have dimension=" << dimension << ", here dimension=" << point.getDimension();
  Scalar logSurvival = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i)
  {
    const Scalar x = std::max(point[i], a_[i]);
    if (x >= b_[i]) return 0.0;
    logSurvival += std::log(b_[i] - x);
  }
  return std::exp(logSurvival + logNormalizationFactor_);
}
...
Scalar MultivariateUniform::computeProbability(const Interval & interval) const
{
  ...
  for (UnsignedInteger i = 0; i < dimension; ++i)
  {
    ...
    logProbability += std::log(upper - lower);
  }
  return std::exp(logProbability + logNormalizationFactor_);
}
    ...
    Scalar & marginalProb) const
{
  ...
  marginalProb = std::pow(prob, 1.0 / dimension);
  if (tail)
  {
    for (UnsignedInteger i = 0; i < dimension; ++i)
      quantile[i] = b_[i] - marginalProb * (b_[i] - a_[i]);
  }
  else
  {
    for (UnsignedInteger i = 0; i < dimension; ++i)
      quantile[i] = a_[i] + marginalProb * (b_[i] - a_[i]);
  }
  return quantile;
}
...
void MultivariateUniform::setParameter(const Point & parameter)
{
  ...
  Point a(dimension);
  Point b(dimension);
  std::copy(parameter.begin(), parameter.begin() + dimension, a.begin());
  std::copy(parameter.begin() + dimension, parameter.end(), b.begin());
