...
{
  return {1.0 * k_, 1.0 * n_};
}

void CombinationsDistribution::setParameter(const Point & parameter)
{
  if (parameter.getSize() != 2) throw InvalidArgumentException(HERE) << "Error: expected 2 values, got " << parameter.getSize();
  if (parameter[0] != std::round(parameter[0])) throw InvalidArgumentException(HERE) << "Error: the first parameter k must be an integer, got " << parameter[0];
  if (parameter[1] != std::round(parameter[1])) throw InvalidArgumentException(HERE) << "Error: the second parameter n must be an integer, got " << parameter[1];
  const Scalar w = getWeight();
  *this = CombinationsDistribution(static_cast<UnsignedInteger>(parameter[0]), static_cast<UnsignedInteger>(parameter[1]));
  setWeight(w);
}
