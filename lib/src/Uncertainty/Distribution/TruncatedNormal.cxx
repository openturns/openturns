  ...
  , normalizationFactor_(0.0)
{
  ...
  phiANorm_ = SpecFunc::ISQRT2PI * SpecFunc::Exp(-0.5 * aNorm_ * aNorm_);
  phiBNorm_ = SpecFunc::ISQRT2PI * SpecFunc::Exp(-0.5 * bNorm_ * bNorm_);
  computeRange();
}
...
Point TruncatedNormal::computeDDF(const Point & point) const
{
  ...
  return Point(1, -normalizationFactor_ * xNorm * SpecFunc::ISQRT2PI * SpecFunc::Exp(-0.5 * xNorm * xNorm) * iSigma * iSigma);
}
...
Scalar TruncatedNormal::computePDF(const Point & point) const
{
  ...
  const Scalar iSigma = 1.0 / sigma_;
  const Scalar xNorm = (x - mu_) * iSigma;
  return normalizationFactor_ * SpecFunc::Exp(-0.5 * xNorm * xNorm) * SpecFunc::ISQRT2PI * iSigma;
}
...
Point TruncatedNormal::computePDFGradient(const Point & point) const
{
  ...
  const Scalar factPhiXNorm = SpecFunc::Exp(-0.5 * xNorm * xNorm) * SpecFunc::ISQRT2PI * iDenom2;
...
Point TruncatedNormal::computeCDFGradient(const Point & point) const
{
  ...
  const Scalar phiXNorm = SpecFunc::Exp(-0.5 * xNorm * xNorm) * SpecFunc::ISQRT2PI;
