  ...
  , conditioningValues_(conditioningValues)
{
  ...
  if (!distribution.isDiscrete() && !distribution.isContinuous())
  ...
  /** Integration algorithm used to compute the CDF. By default it is an IteratedQuadrature. */
  integrationAlgorithm_ = IteratedQuadrature(GaussKronrod(ResourceMap::GetAsUnsignedInteger("PointConditionalDistribution-MaximumSubIntervals"),
                                                          ResourceMap::GetAsScalar("PointConditionalDistribution-MaximumError"),
                                                          GaussKronrod::GetRuleFromName(ResourceMap::GetAsString("PointConditionalDistribution-Rule"))));
  update();
}
...
class PointConditionalDistributionVBoundEvaluation : public EvaluationImplementation
{
  ...
  {
    const UnsignedInteger dimension = distribution_.getDimension();
    const Scalar value = std::pow(distribution_.computePDF(inP), r_ / (1.0 + r_ * dimension));
    return inP * value;
  }
...
void PointConditionalDistribution::update()
{
  ...
  if (!(logNormalizationFactor_ > std::log(getPDFEpsilon())))
    throw InvalidArgumentException(HERE) << "Conditioning vector PDF value is too low";
...
Point PointConditionalDistribution::getRealization() const
{
  ...
  if (dimension <= ResourceMap::GetAsUnsignedInteger("PointConditionalDistribution-SmallDimension"))
  {
    ...
    {
      // initialize the method here and not in the update() method to avoid calls on marginals through computeMean
      // search on the resticted range
      const Point mu(getMean());
      const Point stddev(getStandardDeviation());
      const Interval bounds(getRange().intersect(Interval(mu - 3.0 * stddev, mu + 3.0 * stddev)));
      ...
      OptimizationAlgorithm algo(OptimizationAlgorithm::Build("Cobyla"));
      algo.setProblem(problemU);
      algo.setStartingPoint(mu);
      algo.run();
      ...
      for (UnsignedInteger i = 0; i < dimension; ++ i)
      {
        ...
        problemVI.setBounds(bounds);
        algo.setProblem(problemVI);
        algo.setStartingPoint(mu);
        algo.run();
        supV_[i] = algo.getResult().getOptimalValue()[0];
        if (bounds.getLowerBound()[i] < 0.0)
        {
          problemVI.setMinimization(true);
          algo.setProblem(problemVI);
          algo.run();
          infV_[i] = algo.getResult().getOptimalValue()[0];
        }
      }
    }
...
Scalar PointConditionalDistribution::computeCDF(const Point & point) const
{
  ...
  if (isContinuous())
  {
    ...
    {
      // Monte Carlo for high dimension
      const Interval domain(getRange().getLowerBound(), point);
      const DomainEvent event(RandomVector(marginalConditionedDistribution_), domain);
      const UnsignedInteger samplingSize = ResourceMap::GetAsUnsignedInteger("PointConditionalDistribution-CDFSamplingSize");
      cdfValue = event.getSample(samplingSize).computeMean()[0] / std::exp(logNormalizationFactor_);
    }
  }
  else
...
Scalar PointConditionalDistribution::computeScalarQuantile(const Scalar prob, const Bool tail) const
{
  ...
  return marginalConditionedDistribution_.computeScalarQuantile(prob, tail);
}
...
Distribution PointConditionalDistribution::getMarginal(const UnsignedInteger index) const
{
  ...
  return DistributionImplementation::getMarginal(index);
}
...
{
  if (useSimplifiedVersion_)
    return simplifiedVersion_.computePDFGradient(point);
...
{
  if (useSimplifiedVersion_)
    return simplifiedVersion_.computeCDFGradient(point);
