  ...
  : MarginalDistribution(UniformOrderStatistics(n), indices)
{
  ...
  if (!indices.isStrictlyIncreasing()) throw InvalidArgumentException(HERE) << "Error: expected an increasing list of marginal indices, here indices=" << indices;
...
Distribution MarginalUniformOrderStatistics::getMarginal(const Indices & indices) const
{
  ...
  MarginalUniformOrderStatistics marginal(getN(), marginalIndices);
...
/* String converter */
String MarginalUniformOrderStatistics::__repr__() const
{
  OSS oss(true);
  oss << "class=" << MarginalUniformOrderStatistics::GetClassName()
      << " name=" << getName()
...
String MarginalUniformOrderStatistics::__str__(const String & ) const
{
  OSS oss(false);
  oss << getClassName() << "(n = " << getN() << ", indices = " << indices_ << ")";
  return oss;
}
