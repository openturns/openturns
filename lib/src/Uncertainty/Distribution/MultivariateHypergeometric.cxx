...
/* Parameters constructor with total population */
MultivariateHypergeometric::MultivariateHypergeometric(const UnsignedInteger n,
                                                       const Point & colors,
                                                       const UnsignedInteger total)
  : DistributionImplementation()
  , n_(0)
  , colors_(0)
  , total_(0.0)
...
Bool MultivariateHypergeometric::operator ==(const MultivariateHypergeometric & other) const
{
  ...
  return hasEqualBase(other) && (n_ == other.n_) && (colors_ == other.colors_) && (total_ == other.total_);
}
...
{
  OSS oss;
  oss << getClassName() << "(n = " << n_ << ", colors = " << colors_.__str__() << ", total = " << total_ << ")";
  return oss;
}
...
Scalar MultivariateHypergeometric::computePDF(const Point & point) const
{
  ...
  const Scalar rest = total_ - sumColors;
  const UnsignedInteger restDraws = n_ - sumX;
  if (restDraws > rest) return 0.0;
  ...
  if (rest > 0.0)
  {
    ...
    logPDF += SpecFunc::LogGamma(rest + 1.0) - SpecFunc::LogGamma(restDrawsP + 1.0) - SpecFunc::LogGamma(rest - restDrawsP + 1.0);
  }
...
Scalar MultivariateHypergeometric::computeCDF(const Point & point) const
{
  ...
  // Early exit for 1D case, which reduces to the univariate hypergeometric distribution
  if (dimension == 1)
  {
  ...
  Scalar sumColors = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i) sumColors += colors_[i];
  const Scalar rest = total_ - sumColors;
  if (rest > 0.0)
  {
    Point fullColors(colors_);
    fullColors.add(rest);
    const MultivariateHypergeometric full(n_, fullColors);
...
Scalar MultivariateHypergeometric::computeProbability(const Interval & interval) const
{
  ...
  Scalar sumColorsProb = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i) sumColorsProb += colors_[i];
  if (sumColorsProb < total_)
  {
    Point fullColors(colors_);
    fullColors.add(total_ - sumColorsProb);
    Point lower(interval.getLowerBound());
    Point upper(interval.getUpperBound());
    lower.add(0.0);
    upper.add(1.0 * n_);
    return MultivariateHypergeometric(n_, fullColors).computeProbability(Interval(lower, upper));
  }
  Point lower(interval.getLowerBound());
  Point upper(interval.getUpperBound());
...
Distribution MultivariateHypergeometric::getMarginal(const Indices & indices) const
{
  ...
  // General case, mimicking Multinomial: the marginal of a subset is a defective multivariate hypergeometric
  // with the same number of draws and total population, tracking only the selected colors
  const UnsignedInteger total = static_cast<UnsignedInteger>(total_);
  MultivariateHypergeometric marginal(n_, colors_.select(indices), total);
...
void MultivariateHypergeometric::setParameter(const Point & parameter)
{
  ...
  if (parameter.getDimension() != (dimension + 2))
    throw InvalidArgumentException(HERE) << "MultivariateHypergeometric expected " << (dimension + 2) << " parameters, got " << parameter.getDimension();
  ...
  if (parameter[1] != std::round(parameter[1])) throw InvalidArgumentException(HERE) << "the MultivariateHypergeometric second parameter total must be an integer, got " << parameter[1];
  ...
  std::copy(parameter.begin() + 2, parameter.end(), colors.begin());
  // Mimicking Multinomial renormalization: when the total is smaller than the sum, use the sum (classical case)
  Scalar sum = 0.0;
  for (UnsignedInteger i = 0; i < dimension; ++i) sum += colors[i];
  UnsignedInteger total = static_cast<UnsignedInteger>(parameter[1]);
  if (1.0 * total < sum) total = static_cast<UnsignedInteger>(std::round(sum));
  if (1.0 * total < parameter[0]) throw InvalidArgumentException(HERE) << "the total population = " << total << " must be greater or equal to the number of draws n = " << parameter[0];
  const Scalar eta = eta_;
  const Scalar smallA = smallA_;
  const Scalar w = getWeight();
  const Description d(getDescription());
  *this = MultivariateHypergeometric(static_cast<UnsignedInteger>(parameter[0]), colors, total);
  setSmallA(smallA);
  setEta(eta);
  setWeight(w);
  setDescription(d);
...
void MultivariateHypergeometric::setColors(const Point & colors)
{
  ...
  // Mimicking Multinomial: the tracked colors may sum to less than the total population,
  ...
  if (total_ < sum) total_ = sum;
  if (sum > total_) throw InvalidArgumentException(HERE) << "the sum of colors = " << sum << " must be less or equal to the total population = " << total_;
  if (total_ < n_ * (1.0 - supportEpsilon_)) throw InvalidArgumentException(HERE) << "the total population = " << total_ << " must be greater or equal to the number of draws n = " << n_;
  colors_ = colors;
  updateSpectral();
...
/* Total population accessor */
void MultivariateHypergeometric::setTotal(const UnsignedInteger total)
{
  Scalar sum = 0.0;
  for (UnsignedInteger i = 0; i < colors_.getDimension(); ++i) sum += colors_[i];
  if (1.0 * total < sum) throw InvalidArgumentException(HERE) << "the total population = " << total << " must be greater or equal to the sum of colors = " << sum;
  if (1.0 * total < n_) throw InvalidArgumentException(HERE) << "the total population = " << total << " must be greater or equal to the number of draws n = " << n_;
  if (total != static_cast<UnsignedInteger>(total_))
  {
    total_ = 1.0 * total;
    updateSpectral();
    isAlreadyComputedMean_ = false;
...
/* Total population accessor */
UnsignedInteger MultivariateHypergeometric::getTotal() const
{
  return static_cast<UnsignedInteger>(total_);
}
...
void MultivariateHypergeometric::setN(const UnsignedInteger n)
{
  ...
  if (total_ > 0.0)
  {
    if (n > total_) throw InvalidArgumentException(HERE) << "Error: the number of draws n = " << n << " must be smaller or equal to the total population = " << total_ << ".";
  }
  if (n != n_)
  {
    n_ = n;
