...
void KarhunenLoeveSVDAlgorithm::run()
{
  ...
  if (ResourceMap::GetAsBool("KarhunenLoeveSVDAlgorithm-UseRandomSVD"))
  {
    ...
    if (ResourceMap::Get("KarhunenLoeveSVDAlgorithm-RandomSVDVariant") == "redsvd")
    {
      LOGINFO("Use redsvd variant");
    ...
    {
      LOGINFO("Use Tygert variant");
      const UnsignedInteger l = rank + ResourceMap::GetAsUnsignedInteger("KarhunenLoeveSVDAlgorithm-TygertMargin");
      const UnsignedInteger iMax = ResourceMap::GetAsUnsignedInteger("KarhunenLoeveSVDAlgorithm-TygertIterations");
      // Create a Gaussian mixing matrix to apply to A = designMatrix
      LOGINFO("Create a Gaussian mixing matrix to apply to A = designMatrix");
