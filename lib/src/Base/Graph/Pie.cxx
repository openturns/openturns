...
void Pie::setData(const Point & data)
{
  ...
  // Filter out values too small to display, aggregate them
  const UnsignedInteger size = data.getSize();
  Point filteredData;
  Description filteredLabels;
  Description filteredOutLabels;
  Scalar otherSum = 0.0;
  const Scalar epsilon = ResourceMap::GetAsScalar("Pie-SmallValue");
  const UnsignedInteger maxLabelSize = ResourceMap::GetAsUnsignedInteger("Pie-LabelEllipsisSize");
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    if (data[i] > epsilon)
    {
    ...
    else
    {
      otherSum += data[i];
      if (labels_.getSize())
        filteredOutLabels.add(labels_[i]);
  ...
  if (otherSum > 0.0)
  {
    ...
    if (labels_.getSize())
    {
      String otherLabel;
      Bool fits = true;
      for (UnsignedInteger i = 0; i < filteredOutLabels.getSize(); ++i)
      {
        const String & label = filteredOutLabels[i];
        const UnsignedInteger newSize = otherLabel.size() + label.size() + std::min(i, 1ul);
        if (newSize > maxLabelSize)
        {
          fits = false;
          break;
        }
      ...
      filteredLabels.add(fits ? otherLabel : otherLabel + "...");
    }
  }
  if (labels_.getSize())
