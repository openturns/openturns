...
Bool SobolSequence::operator ==(const SobolSequence & other) const
{
  if (this == &other) return true;
  if (!hasEqualBase(other)) return false;
  if (scrambling_ != other.scrambling_) return false;
  if (multidigitBits_ != other.multidigitBits_) return false;
...
Bool SobolSequence::equals(const LowDiscrepancySequenceImplementation & other) const
{
  const SobolSequence * p_other = dynamic_cast<const SobolSequence *>(&other);
