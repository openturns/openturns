...
{
  const DualLinearCombinationEvaluation * p_other = dynamic_cast<const DualLinearCombinationEvaluation *>(&other);
...
void DualLinearCombinationEvaluation::setParameter(const Point & parameter)
{
  ...
  if (parameter.getSize() != parameterSize)
    throw InvalidDimensionException(HERE) << "DualLinearCombinationEvaluation expected a parameter of size " << parameterSize
