...
#else
  const NumericalScalar absZ(abs(z));
  if (absZ >= SpecFunc::LogMaxNumericalScalar)
    {
      const NumericalComplex mainPart(exp(z) / z);
      if (imag(z) > 0.0) return mainPart + NumericalComplex(0.0, M_PI);
      if (imag(z) < 0.0) return mainPart + NumericalComplex(0.0, -M_PI);
      return mainPart;
    }
  if (absZ > 2.0 - 1.035 * std::log(SpecFunc::Precision)) return EiAsymptoticSeries(z);
  if ((absZ > 1.0) && ((real(z) < 0.0) || (abs(imag(z)) > 1.0))) return EiContinuedFraction(z);
...
{
  return real(Ei(NumericalComplex(z, 0.0)));
}
