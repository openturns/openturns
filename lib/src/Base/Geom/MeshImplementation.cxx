//                                               -*- C++ -*-
/**
 *  @brief MeshImplementation is defined as a collection of n-D vertices and simplices
 *
...
CLASSNAMEINIT(MeshImplementation)

static const Factory<MeshImplementation> Factory_MeshImplementation;

/* Default constructor */
MeshImplementation::MeshImplementation(const UnsignedInteger dimension)
  : PersistentObject()
...
MeshImplementation::MeshImplementation(const Sample & vertices)
  : PersistentObject()
...
/* Clone method */
MeshImplementation * MeshImplementation::clone() const
{
  return new MeshImplementation(*this);
}

/* Dimension accessor */
UnsignedInteger MeshImplementation::getDimension() const
{
  return vertices_.getDimension();
}

/* Description of the vertices accessor */
Description MeshImplementation::getDescription() const
{
  return vertices_.getDescription();
}

/* Vertices accessor */
Sample MeshImplementation::getVertices() const
{
  return vertices_;
}

void MeshImplementation::setVertices(const Sample & vertices)
{
  vertices_ = vertices;
...
void MeshImplementation::computeKDTree()
{
  tree_ = KDTree(vertices_);
}

/* Vertex accessor */
Point MeshImplementation::getVertex(const UnsignedInteger index) const
{
...
void MeshImplementation::setVertex(const UnsignedInteger index,
                     const Point & vertex)
{
...
/* Simplices accessor */
MeshImplementation::IndicesCollection MeshImplementation::getSimplices() const
{
  return simplices_;
}

void MeshImplementation::setSimplices(const IndicesCollection & simplices)
{
...
/* Simplex accessor */
Indices MeshImplementation::getSimplex(const UnsignedInteger index) const
{
...
void MeshImplementation::checkValidity() const
{
...
Bool MeshImplementation::isValid() const
{
  Bool result = true;
  try
  {
...
Bool MeshImplementation::contains(const Point & point) const
{
...
SquareMatrix MeshImplementation::buildSimplexMatrix(const UnsignedInteger index) const
{
  if (index >= getSimplicesNumber()) throw InvalidArgumentException(HERE) << "Error: the simplex index=" << index << " must be less than the number of simplices=" << getSimplicesNumber();
  ...
  for (UnsignedInteger j = 0; j <= dimension; ++j)
  {
    ...
    for (UnsignedInteger i = 0; i < dimension; ++i) matrix(i, j) = vertexJ[i];
    matrix(dimension, j) = 1.0;
  }
  return matrix;
}

/* Check if the given point is in the given simplex */
Bool MeshImplementation::checkPointInSimplex(const Point & point,
...
Bool MeshImplementation::checkPointInSimplexWithCoordinates(const Point & point,
    ...
    Point & coordinates) const
{
  ...
  const UnsignedInteger dimension(getDimension());
  for (UnsignedInteger i = 0; i <= dimension; ++i) if ((coordinates[i] < 0.0) || (coordinates[i] > 1.0)) return false;
  return true;
}

/* Get the number of vertices */
UnsignedInteger MeshImplementation::getVerticesNumber() const
{
  return vertices_.getSize();
}

/* Get the number of simplices */
UnsignedInteger MeshImplementation::getSimplicesNumber() const
{
...
{
  const MeshImplementation & mesh_;
  const Point & point_;
  Scalar minDistance_;
  UnsignedInteger minIndex_;

  NearestFunctor(const MeshImplementation & mesh, const Point & point)
...
UnsignedInteger MeshImplementation::getNearestVertexIndex(const Point & point) const
{
...
Indices MeshImplementation::getNearestVertexAndSimplexIndicesWithCoordinates(const Point & point,
    Point & coordinates) const
{
  if (point.getDimension() != getDimension()) throw InvalidArgumentException(HERE) << "Error: expected a point of dimension " << getDimension() << ", got a point of dimension " << point.getDimension();
...
Point MeshImplementation::getNearestVertex(const Point & point) const
{
...
Indices MeshImplementation::getNearestVertexIndex(const Sample & points) const
{
...
Scalar MeshImplementation::computeSimplexVolume(const UnsignedInteger index) const
{
  if (index >= getSimplicesNumber()) throw InvalidArgumentException(HERE) << "Error: the simplex index=" << index << " must be less than the number of simplices=" << getSimplicesNumber();
  ...
  SquareMatrix matrix(buildSimplexMatrix(index));
  Scalar sign = 0.0;
  return exp(matrix.computeLogAbsoluteDeterminant(sign, false) - SpecFunc::LogGamma(getDimension() + 1));
}

/* Compute P1 gram matrix */
CovarianceMatrix MeshImplementation::computeP1Gram() const
{
...
Bool MeshImplementation::isRegular() const
{
...
Point MeshImplementation::getLowerBound() const
{
  return vertices_.getMin();
}

/* Upper bound of the bounding box */
Point MeshImplementation::getUpperBound() const
{
...
MeshImplementation::IndicesCollection MeshImplementation::getVerticesToSimplicesMap() const
{
 ...
 */
Point MeshImplementation::computeWeights() const
{
  ...
  weights /= (getDimension() + 1.0);
  return weights;
}

/* Comparison operator */
Bool MeshImplementation::operator == (const MeshImplementation & other) const
{
...
/* String converter */
String MeshImplementation::__repr__() const
{
...
String MeshImplementation::__str__(const String & offset) const
{
  return __repr__();
}

/* Drawing method */
Graph MeshImplementation::draw() const
{
  const UnsignedInteger dimension(getDimension());
  if (dimension > 3) throw InvalidArgumentException(HERE) << "Error: cannot draw a mesh of dimension > 3.";
  if (dimension == 1) return draw1D();
  if (dimension == 2) return draw2D();
  if (dimension == 3) return draw3D();
  return Graph();
}

Graph MeshImplementation::draw1D() const
{
  checkValidity();
  if (getDimension() != 1) throw InvalidArgumentException(HERE) << "Error: cannot draw a mesh of dimension different from 1 with the draw1D() method.";
  ...
  Graph graph(String(OSS() << "MeshImplementation " << getName()), "x", "y", true, "topright");
  // The vertices
...
Graph MeshImplementation::draw2D() const
{
  checkValidity();
  const UnsignedInteger verticesSize = getVerticesNumber();
  const UnsignedInteger simplicesSize = getSimplicesNumber();
...
Graph MeshImplementation::draw3D(const Bool drawEdge,
                   const Scalar thetaX,
                   const Scalar thetaY,
                   const Scalar thetaZ,
                   const Bool shading,
...
MeshImplementation MeshImplementation::ImportFromMSHFile(const String & fileName)
{
  ...
  file.close();
  return MeshImplementation(vertices, simplices);
}

/* VTK export */
String MeshImplementation::streamToVTKFormat() const
{
  const UnsignedInteger dimension(getDimension());
  if (dimension > 3) throw InvalidDimensionException(HERE) << "Error: cannot export a mesh of dimension=" << dimension << " into the VTK format. Maximum dimension is 3.";
  ...
  while ((verticesPerSimplex <= dimension) && (simplices_[0][verticesPerSimplex] != lastIndex))
  {
...
void MeshImplementation::exportToVTKFile(const String & fileName) const
{
...
void MeshImplementation::save(Advocate & adv) const
{
...
void MeshImplementation::load(Advocate & adv)
{
