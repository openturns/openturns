...
int main(int, char *[])
{
  ...
  try
  {
    ...
    ResourceMap::SetAsUnsignedInteger("HMatrix-MaxLeafSize", 225);
    ResourceMap::SetAsUnsignedInteger("HMatrix-ClusteringMaxDof", 225);
    HMatrixFactory hmatrixFactory;

    const UnsignedInteger n = 29;
    ...
    hmatrixParameters.setAssemblyEpsilon(1.e-8);
    hmatrixParameters.setRecompressionEpsilon(1.e-8);
    hmatrixParameters.setAdmissibility("hodlr");
    hmatrixParameters.setFactorizationMethod("hodlr");

    HMatrix hmat(hmatrixFactory.build(vertices, 1, true, hmatrixParameters));
    ...
    Scalar threshold = 2.e-3;
    assert_almost_equal(std::abs(diffNorm - threshold * rhsCopyNorm), 0.00246, 1e-5, 1e-5);
  }
  catch (TestFailed & ex)
  {
