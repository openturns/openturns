...
int main(int, char *[])
{
  ...
  try
  {
    ...
    // Naive leave-one-out
    Sample residualsLOOSample(samplingSize, outputDimension);
    ...
    for (UnsignedInteger k = 0; k < outputDimension; ++k)
    {
      ...
      mseLOOnaive[k] = residualsLOO.normSquare() / samplingSize;
    }
    ...
    // Naive KFold
    Sample squaredResidualsKFold(samplingSize, outputDimension);
