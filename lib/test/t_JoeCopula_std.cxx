//                                               -*- C++ -*-
/**
 *  @brief The test file of class JoeCopula for standard methods
 *
...
int main(int, char *[])
{
  ...
  try
  {
    ...
    JoeCopula copula(0.5, 0.5, 0.5);
    Scalar myTheta = copula.getTheta();
    copula.setTheta(myTheta);
    copula.setName("a Joe copula");
    ...
    const Point oneRealization = copula.getRealization();
    fullprint << "oneRealization=" << oneRealization << std::endl;

    // Test for sampling
    UnsignedInteger size = 10;
    const Sample oneSample = copula.getSample( size );
    fullprint << "oneSample=" << oneSample << std::endl;

    // Test for sampling
    size = 10000;
    const Sample anotherSample = copula.getSample( size );
    ...
    Point y(1, 0.5);
    Scalar x = 0.6;
    Scalar p = copula.computeConditionalCDF(x, y);
    fullprint << "CDF(x|y)=" << p << std::endl;
    fullprint << "Quantile(p|y)=" << copula.computeConditionalQuantile(p, y) << std::endl;
