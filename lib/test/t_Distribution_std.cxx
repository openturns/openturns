...
int main(int, char *[])
{
  ...
  try
  {
    ...
    assert_equal(Triangular{} != DistributionImplementation(Gumbel{}), true);
    assert_equal(Gumbel{} != DistributionImplementation(Triangular{}), true);
    assert_equal(Normal(2) == Normal(2), true);
    assert_equal(Normal(2) != Normal(3), true);
#if !(defined(__clang__) && (__cplusplus >= 202002L)) // error: use of overloaded operator '==' is ambiguous
