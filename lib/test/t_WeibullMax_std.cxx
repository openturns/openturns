...
class TestObject : public WeibullMax
{
public:
  TestObject() : WeibullMax(2.0, 1.5, -0.5) {}
  virtual ~TestObject() {}
};
...
int main(int, char *[])
{
  ...
  try
  {
    ...
    WeibullMax distribution(2.0, 1.5, -0.5);
    ...
    fullprint << "variance=" << oneSample.computeVariance() << std::endl;
    ...
    fullprint << "ddf (FD)=" << (distribution.computePDF( point + Point(1, eps) ) - distribution.computePDF( point  + Point(1, -eps) )) / (2.0 * eps) << std::endl;
    ...
    WeibullMax::PointWithDescriptionCollection parameters = distribution.getParametersCollection();
