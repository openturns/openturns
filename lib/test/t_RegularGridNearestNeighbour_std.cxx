...
int main(int argc, char *argv[])
{
  ...
  const RegularGridNearestNeighbour regularGridNearestNeighbour(regularGrid);
  fullprint << "regularGridNearestNeighbour=" << regularGridNearestNeighbour << std::endl;
  const Sample test(Normal(1).getSample(20));
  for (UnsignedInteger i = 0; i < test.getSize(); ++i)
  {
    const UnsignedInteger index = regularGridNearestNeighbour.getNearestNeighbourIndex(test[i]);
    const Point neighbour(regularGridNearestNeighbour.getNearestNeighbour(test[i]));
  ...
  for (UnsignedInteger i = 0; i < test.getSize(); ++i)
  {
    const Indices indices = regularGridNearestNeighbour.getNearestNeighboursIndices(test[i], k, true);
  ...
  Indices indices = regularGridNearestNeighbour.getNearestNeighboursIndices(test[0], n, true);
