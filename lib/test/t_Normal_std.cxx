  ...
  try
  {
    ...
    for (UnsignedInteger dim = 1; dim <= 4; dim++)
    {
      ...
      assert_almost_equal((chol * invChol), IdentityMatrix(dim));
      {
        ...
        const Bool equal = distribution == EllipticalDistribution(Student(4.5, meanPoint, sigma, R));
        fullprint << "Comparison with an EllipticalDistribution distribution " << equal << std::endl;
      }
      {
        // Comparison with a non-elliptical distribution
        const Bool equal = distribution == DistributionImplementation(Exponential{});
