//                                               -*- C++ -*-
/**
 *  @brief The test file of class MultivariatePolya for standard methods
 *
...
int main(int, char *[])
{
  ...
  try
  {
    ...
    Interval interval(Point(distribution.getDimension(), 0.0), Point(distribution.getDimension(), 2.0));
    ...
    MultivariatePolya::PointWithDescriptionCollection parameters = distribution.getParametersCollection();
    fullprint << "parameters=" << parameters << std::endl;
    Distribution marginal(distribution.getMarginal(0));
    fullprint << "marginal(0)=" << marginal << std::endl;
    // Degenerate case c=0 must exactly match the multinomial distribution
    MultivariatePolya degenerate(5, Point(3, 0.25), 0.0);
    Multinomial multinomial(5, Point(3, 1.0 / 3.0));
    fullprint << "degenerate pdf=" << degenerate.computePDF(point) << " multinomial pdf=" << multinomial.computePDF(point) << std::endl;
    fullprint << "degenerate cdf=" << degenerate.computeCDF(Point{1.0, 3.0, 1.0}) << " multinomial cdf=" << multinomial.computeCDF(Point{1.0, 3.0, 1.0}) << std::endl;
    // One-dimensional distribution is degenerate at n
    MultivariatePolya oneDimensional(4, Point{5.0}, 2.0);
    fullprint << "1d cdf(10)=" << oneDimensional.computeCDF(Point{10.0}) << " cdf(3)=" << oneDimensional.computeCDF(Point{3.0}) << " pdf(4)=" << oneDimensional.computePDF(Point{4.0}) << std::endl;
  }
  catch (TestFailed & ex)
  {
