...
int main(int, char *[])
{
  ...
  try
  {
    // Testing four algorithms on a function from R^3 to R.
    {
      ...
      const Point lbIntegration({0., 0., 0.});
      const Point ubIntegration({1., 1., 1.});
    ...
    {
      const SymbolicFunction f({"x", "y", "z"}, {"exp(-(x * x + y * y + z *z))", "(x / 2 * x / 2 + y / 3 * y / 3 + z / 4 * z / 4 - 1.125)"});
      const Point lbIntegration({0., 0., 1.});
      const Point ubIntegration({2., 3., 4.});
