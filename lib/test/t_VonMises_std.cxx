...
class TestObject : public VonMises
{
public:
  TestObject() : VonMises(-0.5, 1.5) {}
  virtual ~TestObject() {}
};
...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    PDFgrFD[0] = (VonMises(distribution.getMu() + eps, distribution.getKappa()).computePDF(point) -
                  VonMises(distribution.getMu() - eps, distribution.getKappa()).computePDF(point)) / (2.0 * eps);
    PDFgrFD[1] = (VonMises(distribution.getMu(), distribution.getKappa() + eps).computePDF(point) -
                  VonMises(distribution.getMu(), distribution.getKappa() - eps).computePDF(point)) / (2.0 * eps);
    ...
    CDFgrFD[0] = (VonMises(distribution.getMu() + eps, distribution.getKappa()).computeCDF(point) -
                  VonMises(distribution.getMu() - eps, distribution.getKappa()).computeCDF(point)) / (2.0 * eps);
    CDFgrFD[1] = (VonMises(distribution.getMu(), distribution.getKappa() + eps).computeCDF(point) -
                  VonMises(distribution.getMu(), distribution.getKappa() - eps).computeCDF(point)) / (2.0 * eps);
    ...
    VonMises::NumericalPointWithDescriptionCollection parameters = distribution.getParametersCollection();
