...
:ref:`Taylor Expansion <taylor_expansion>`. Let :math:`\inputRV = (X_1, \dots, X_\inputDim)` be the input random vector of dimension :math:`\inputDim`.
...
for :math:`i \in \{1, ..., \inputDim\}` where :math:`\sigma_i^2 = \Var{X_i}`. In that case,
we have :math:`0 \leq \eta_i \leq 1`. These importance factors are also called *importance factors derived from perturbation methods* and defined in [daveiga2022]_ (see eq. 2.6 page 38).
