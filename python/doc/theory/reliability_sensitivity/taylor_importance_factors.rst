...
We consider the Taylor expansion of a function: Refer to :ref:`Taylor Expansion <taylor_expansion>` for
details on the expressions of the first-order and second-order Taylor expansions. Let
:math:`\inputRV = (X_1, \dots, X_\inputDim)` be the
...
Refer to :ref:`Taylor Expansion Moments <taylor_expansion_moments>` for
details on the approximations of the mean and the variance of the output random variable :math:`Y`.

The importance factor of :math:`X_i`, for :math:`i \in \{1, ..., \inputDim\}`, is defined by:

.. math::
    :label: eta_i

    \eta_i = \frac{ \frac{\partial \model}{\partial x_i}(\vect{\mu}) }{\Var Y} \sum_{j=1}^\inputDim
    \frac{\partial \model}{\partial x_j}(\vect{\mu})c_{ij}.


where :math:`c_{ij} = \Cov{X_i, X_j}`.
...
where :math:`\sigma_i^2 = c_{ii}`. In that case, we have :math:`0 \leq \eta_i \leq 1`.
These importance factors are also called
