...
This test case is composed of nine random variables distributed according to truncated Normal distributions :math:`\mathcal{TN}(\mu,\sigma,a,b)` with :math:`\mu` the mean, :math:`\sigma` the standard deviation, :math:`a` the lower bound and :math:`b` the upper bound:
