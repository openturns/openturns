...
class GaussianConvolution(ot.OpenTURNSPythonDynamicalFunction):
    ...
    def _exec(self, X):
        ...
        f = ot.NumericalMathFunction(ot.PiecewiseLinearEvaluationImplementation([x[0] for x in inputTG.getVertices()], inputValues))
...
N = 5
X = ot.TemporalNormalProcess(ot.GeneralizedExponential(1, 0.1, 1.0), ot.RegularGrid(-5.0, 0.1, 101))
