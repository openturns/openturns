...
atoms = [ot.Normal([1.0, 2.0], [0.5, 0.8], Id), ot.Normal([1.0, -2.0], [0.9, 0.8], Id), ot.Normal([-1.0, 0.0], [0.5, 0.6], Id)]
weights = [0.3, 0.3, 0.4]
...
graph.setTitle("Mixture classification")
...
for i in range(classes.getSize()):
    index = classes[i]
    graph.add(ot.Cloud([data[i]], palette[index % len(palette)], symbols[index % len(symbols)]))

fig = plt.figure(figsize=(4, 4))
plt.suptitle("MixtureClassifier example")
