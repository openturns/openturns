...
grid.setTitle(r"Bingham($\zeta$, $\Gamma$)")
zeta_list = [
    [0.0, 0.0, 0.0],
    [1.0, 0.5, 0.0],
    [4.0, 1.0, 0.0],
...
for k, zeta in enumerate(zeta_list):
    distribution = otexp.Bingham(zeta, ot.IdentityMatrix(3))
    pdf = distribution.computePDF(ot.Sample(vertices))
    pdf_max = pdf.getMax()[0]
    defVertices = [
        [v[i] * (1.0 + factor * pdf[j, 0] / pdf_max) for i in range(3)]
        for j, v in enumerate(vertices)
    ]
    defMesh = ot.Mesh(ot.Sample(defVertices), simplices)
    field = ot.Field(defMesh, pdf)
    pdf_graph = field.draw()
    pdf_graph.setTitle(rf"$\zeta=({zeta[0]:.3g},{zeta[1]:.3g},{zeta[2]:.3g})$")
    pdf_graph.setXTitle(r"$x$")
    pdf_graph.setYTitle(r"$y$")
    grid.setGraph(k // 3, k % 3, pdf_graph)
