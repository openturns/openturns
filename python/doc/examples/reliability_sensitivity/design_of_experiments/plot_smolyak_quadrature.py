...
# %%
# Similarily, the next function uses the Gauss-Legendre quadrature rule.
...
def drawQuadrature(size_list, abserr_list, quadratureName="Quadrature"):
...
def drawSmolyakQuadrature(level_max):
    ...
    graph = drawQuadrature(size_list, abserr_list, "Smolyak-Gauss")
    return graph
...
def drawTensorizedGaussQuadrature(n_max):
    ...
    for n in range(1, n_max):
        ...
        size_list.append(size)
        abserr_list.append(abserr)
    graph = drawQuadrature(size_list, abserr_list, "Gauss-Legendre")
    return graph
...
palette = ot.DrawableImplementation_BuildDefaultPalette(4)
