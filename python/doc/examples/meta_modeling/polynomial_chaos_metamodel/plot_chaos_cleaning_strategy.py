...
# %%
#
# The next function computes the polynomial chaos :math:`R^2` score using simple validation
...
# will be required in order to produce a polynomial chaos expansion with :math:`R^2`
...
# For each combination, we compute the :math:`R^2` score and select the
# combination with the highest :math:`R^2` coefficient. As shown in [muller2016]_
# page 268, the computed :math:`R^2` may be optimistic, but this is not the
...
# coefficients among the first 101 ones. Let us see the :math:`R^2` score and the
