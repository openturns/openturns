...
ot.ResourceMap.SetAsBool(
    "GaussianProcessFitter-UseAnalyticalAmplitudeEstimate", True
)
...
print(
    "Optimal amplitude = {}".format(gprResult.getCovarianceModel().getAmplitude())
)
...
sample[:, 0] = x_plot
variance = otexp.GaussianProcessConditionalCovariance(gprResult).getConditionalMarginalVariance(x_plot)
sample[:, 1] = variance
...
# krv = ot.ConditionalGaussianProcess(gprResult, grid)
# krv_sample = krv.getSample(5)

# %%

# x_plot = grid.getVertices().asPoint()
...
print(
    "Optimal trend coefficients = {}".format(
        gprResult_nugget.getTrendCoefficients()
    )
)

# %%
gprMeta_nugget = gprResult_nugget.getMetaModel()
gpr_conditional_covariance = otexp.GaussianProcessConditionalCovariance(gprResult_nugget)
variance = [[gpr_conditional_covariance.getConditionalCovariance(xx)[0, 0]] for xx in x_plot]
