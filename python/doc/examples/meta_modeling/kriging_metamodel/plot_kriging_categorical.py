...
# %%
# We define the bending function depending on the beam length L, its height h and the type of cross-section I
# We consider here 3 types of cross-section characterized by differents moments of inertia
def bending(inp):
    L,h,i = inp
    L = L*10+10
    h = h+1
...
l = 3 # Number of discrete levels
dist = ot.ComposedDistribution([ot.Uniform(0,1), ot.Uniform(0,1), ot.UserDefined(np.arange(0,l).reshape((-1,1)))])
...
ki = ot.LatentVariableModel(latdim,l)
...
activecoord = 1 + latdim * (l - 2) # Number of active coordinates in the latent space
...
# We can see that the results are coherent with the function definition, as the levels are mapped on points, 
# the coordinates of which are proportional to the moment of inertia characterizing the different cross-sections
res = algo.getResult()
