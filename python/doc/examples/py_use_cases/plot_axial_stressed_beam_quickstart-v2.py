...
#
# The practical example used is the axial stressed beam described in :ref:`use-case-stressed-beam`.
...
# Build the topical exemple model
...
# %%
limitStateFunction = uc_beam.model
...
# %%
R = uc_beam.distribution_R #ot.LogNormalMuSigma(3.e6, 3.e5, 0.0).getDistribution()
...
# %%
F = uc_beam.distribution_F #ot.Normal(750., 50.)
...
# %%
myDistribution = uc_beam.distribution #ot.ComposedDistribution([R, F])
...
# %%
inputRandomVector = uc_beam.vect #ot.RandomVector(myDistribution)

# %%
# Finally we create a `CompositeRandomVector` by associating the limit state function with the input random vector.

# %%
outputRandomVector = uc_beam.G #ot.CompositeRandomVector(limitStateFunction, inputRandomVector)
...
# %%
myEvent = uc_beam.event #ot.ThresholdEvent(outputRandomVector, ot.Less(), 0.0)
