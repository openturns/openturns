...
# %%
# Load the data from the cantilever beam use-case
data = cantilever_beam.CantileverBeam().data
print(data[:5])
...
# %%
# For simplicity in the next steps we assume the distribution is independent
estimated_distribution = ot.JointDistribution(estimated_marginals)
...
sample_x = estimated_distribution.getSample(1000)
sample_y = model(sample_x)

# %%
# Estimate a non-parametric distribution of the output variable
histogram = ot.HistogramFactory().build(sample_y)
...
# Build a metamodel
# -----------------
algo = ot.LeastSquaresExpansion(sample_x, sample_y, estimated_distribution)
algo.run()
metamodel_result = algo.getResult()
metamodel = metamodel_result.getMetaModel()
test_x = estimated_distribution.getSample(1000)
test_y = model(test_x)
...
# ----------------------
N = 5000
experiment = ot.SobolIndicesExperiment(estimated_distribution, N)
sobol_x = experiment.generate()
sobol_y = metamodel(sobol_x)
algo = ot.SaltelliSensitivityAlgorithm(sobol_x, sobol_y, N)
print(algo.getFirstOrderIndices())
print(algo.getTotalOrderIndices())

# %%
# Plot the sensitivity indices
# The most influential variable is `F`, and we see almost no interactions.
graph = algo.draw()
_ = otv.View(graph)
