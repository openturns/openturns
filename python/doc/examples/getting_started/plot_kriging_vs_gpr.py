...
# %%
# The goal of this example is to highlight the main changes between the old API involving kriging and the new one.
#
# It assumes a basic knowledge on gaussian process regression.
# For that purpose, we create a Gaussian process Regression for a function which has scalar real inputs and outputs.
...
krigeageMM = kriging_result.getMetaModel()

# %%
# We observe that the `scale` and `amplitude` hyper-parameters have been optimized by the `run` method.
# The used optimization method (default one) is the :class:`~openturns.TNC`
...
# The used optimization method (default one for this class) is the :class:`~openturns.Cobyla`, which is different from the old API.
...
graph.add(plot_1d_data(x_test, y_test, legend="Exact", color="black", linestyle="dashed"))
graph.add(plot_1d_data(x_train, y_train, type="Cloud", legend="Data", color="red"))
graph.add(plot_1d_data(x_test, krigeageMM(x_test), legend="Kriging", color="blue"))
graph.add(plot_1d_data(x_test, gprMetamodel(x_test), legend="GPR", color="green"))
graph.setAxes(True)
graph.setXTitle("X")
graph.setYTitle("Y")
graph.setLegendPosition("upper right")
...
krigeageMM_wnf = kriging_result_wnf.getMetaModel()
...
graph.add(plot_1d_data(x_test, krigeageMM_wnf(x_test), legend="Kriging", color="blue"))
graph.add(plot_1d_data(x_test, gprMetamodel_wnf(x_test), legend="GPR", color="green"))
graph.setAxes(True)
graph.setAxes(True)
graph.setXTitle("X")
graph.setYTitle("Y")
...
# %%
ci_lower_bound_km, ci_upper_bound_km = computeBoundsConfidenceInterval(krigeageMM(x_test), quantileAlpha, conditional_sigma_kriging)
ci_lower_bound_km_noise, ci_upper_bound_km_noise = computeBoundsConfidenceInterval(krigeageMM_wnf(x_test), quantileAlpha, conditional_sigma_kriging_wnf)
...
graph.add(plot_1d_data(x_test, y_test, legend="Exact", color="black", linestyle="dashed"))
graph.add(plot_1d_data(x_train, y_train, type="Cloud", legend="Data", color="red"))
graph.add(plot_1d_data(x_test, krigeageMM_wnf(x_test), legend="Kriging + noise", color="blue"))
...
krigeageMM_hsn = kriging_result_hsn.getMetaModel()
...
graph.add(plot_1d_data(x_test, y_test, legend="Exact", color="black", linestyle="dashed"))
graph.add(plot_1d_data(x_train, y_train, type="Cloud", legend="Data", color="red"))
graph.add(plot_1d_data(x_test, krigeageMM_hsn(x_test), legend="Kriging+noise", color="green"))
graph.setAxes(True)
graph.setXTitle("X")
graph.setYTitle("Y")
graph.setLegendPosition("upper right")
