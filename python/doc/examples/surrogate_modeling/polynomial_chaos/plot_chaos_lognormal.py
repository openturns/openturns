...
# %%
#
# In this example, we build a polynomial
...
# such that :math:`\overline{\cP} \nsubseteq L^2(\mu)`. This means that
# the functional space generated by this orthonormal polynomial family
# is **not dense** in the Hilbert space :math:`L^2(\mu)`
# of square-integrable functions with respect to :math:`\mu`. In other words,
# it means that the polynomials orthogonal with respect to :math:`\mu` do not form a
...
#
# This situation occurs for example when :math:`\mu`
...
basis_LN = ot.OrthogonalProductPolynomialFactory(
    [otexp.UniVariateDistributionPolynomialFactory(ot.LogNormal())]
)
basis_size_LN = 6
