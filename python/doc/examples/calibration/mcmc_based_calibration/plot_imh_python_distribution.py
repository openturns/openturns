...
# - Case 2: using the Ratio of Uniforms algorithm with the class :class:`~openturns.RatioOfUniforms`
...
class NewDistribution_RoU(ot.PythonDistribution):
    def __init__(self):
        ...
        self.rangeLow_ = 0.0
        self.rangeUp_ = 2.0 * pi
        self.sampler_ = ot.RatioOfUniforms(self.logPDF_, self.getRange())
...
# --------------------------------------------------
# In that case, we want to use the :class:`~openturns.RatioOfUniforms` algorithm to
# sample :math:`p`. We need to compute the function :math:`\log f` and its range.
#
# We create the function :math:`\log f` and the :class:`~openturns.RatioOfUniforms`:
log_UnscaledPDF = ot.ComposedFunction(ot.SymbolicFunction("x", "log(x)"), f)
ratio_algo = ot.RatioOfUniforms(log_UnscaledPDF, range_PDF)
...
# the documenttaion of :class:`~openturns.RatioOfUniforms`):
...
g_IMH.setXTitle("x")
g_IMH.setLegendPosition("upper right")
view = otv.View(g_IMH)
...
g_RWMH.setXTitle("x")
g_RWMH.setLegendPosition("upper right")
