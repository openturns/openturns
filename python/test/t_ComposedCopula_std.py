...
copulas = ot.CopulaCollection(
    [ot.FrankCopula(3.0), ot.NormalCopula(R), ot.ClaytonCopula(2.0)])
test_distribution = ot.ComposedDistribution([ot.Normal()] * 2)
sample_size = 10
sample = test_distribution.getSample(sample_size)
myEstimatedDist = ot.KernelSmoothing().build(sample)
sklarCopula = myEstimatedDist.getCopula()
copulas.add(sklarCopula)

# getMarginal to merge NormalCopula
copula = ot.ComposedCopula([ot.NormalCopula(R)] * 2)
