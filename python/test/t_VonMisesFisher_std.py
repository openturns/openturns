...
ott.assert_almost_equal(distUniform.computeEntropy(), 2.531024, 1e-5, 0.0)
...
dist = otexp.VonMisesFisher(ot.Point([1.0, 0.0]), 2.0)
strRepr = dist.__repr__()
assert "VonMisesFisher" in strRepr
assert "mu" in strRepr
assert "kappa" in strRepr
strStr = dist.__str__()
assert "VonMisesFisher" in strStr
assert "mu" in strStr
assert "kappa" in strStr

# Equality operator
dist1 = otexp.VonMisesFisher(ot.Point([0.0, 1.0]), 2.0)
dist2 = otexp.VonMisesFisher(ot.Point([0.0, 1.0]), 2.0)
dist3 = otexp.VonMisesFisher(ot.Point([0.0, 1.0]), 3.0)
dist4 = otexp.VonMisesFisher(ot.Point([1.0, 0.0]), 2.0)
assert dist1 == dist2
assert not dist1 != dist2
assert dist1 != dist3
assert not dist1 == dist3
assert dist1 != dist4
assert not dist1 == dist4

# Exception tests: methods that throw NotYetImplementedException
point2D = ot.Point([1.0, 0.0])
with ott.assert_raises(RuntimeError):
    distribution.computeCDF(point2D)
with ott.assert_raises(RuntimeError):
    distribution.getCovariance()
with ott.assert_raises(RuntimeError):
    distribution.getStandardDeviation()
with ott.assert_raises(RuntimeError):
    distribution.getSkewness()
with ott.assert_raises(RuntimeError):
    distribution.getKurtosis()

# Exception tests: wrong dimension for computePDF
point3D = ot.Point([1.0, 0.0, 0.0])
with ott.assert_raises(TypeError):
    distribution.computePDF(point3D)
with ott.assert_raises(TypeError):
    distribution.computeLogPDF(point3D)
