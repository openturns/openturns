...
try:
    ...
    computeSO = True
    inputDesign = ot.SaltelliSensitivityAlgorithm.Generate(distribution, size, computeSO)
    outputDesign = model(inputDesign)
    # Case 1 : Estimation of sensitivity using estimator and no bootstrap
    for method in methods:
        sensitivity_algorithm = eval('ot.' + method + "SensitivityAlgorithm(inputDesign, outputDesign, size)")
    ...
    ot.ResourceMap.SetAsBool("MartinezSensitivityAlgorithm-UseAsymptoticInterval", True)
