...
def test_one_input_one_output():
    ...
    values = ot.Matrix(
        [
            [0.8184895, -0.35552172, -0.17465229, 0.046165814, -0.031401148, 0.040513763],
    ...
    print(validCovariance)
    ott.assert_almost_equal(validCovariance - values, nullMatrix, 3e-4, 1e-3)
...
def test_two_outputs():
    ...
    reference_covariance = ot.Matrix(
        [
            [4.4655975, 0.0, 8.3485681, 0.0],
            [0.0, 2.8909595, 0.0, 5.4295163],
            [8.3485681, 0.0, 15.766016, 0.0],
            [0.0, 5.4295163, 0.0, 10.21],
        ]
    )
    ...
    ref_cov_test = ot.Matrix(
        [
            [0.00020797, 2.28332e-20],
            [2.28332e-20, 0.0001365],
        ]
    )
    ...
    ott.assert_almost_equal(covTest, ref_cov_test, 1.0e-3, 1e-3)
