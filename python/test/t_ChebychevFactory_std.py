...
def check_coefficients(factory):
    # Use 10 well-spaced points to validate coefficients up to degree 9
    test_points = [(-0.9 + 0.2 * k) for k in range(10)]
    for i in range(10):
        p = factory.build(i)
...
check_coefficients(chebychev_bounded)
# Affine map from [0, 3] to standard [-1, 1]: a_=2/3, b_=-1
ott.assert_almost_equal(chebychev_bounded.getA(), 2.0 / 3.0)
ott.assert_almost_equal(chebychev_bounded.getB(), -1.0)
