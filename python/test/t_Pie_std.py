...
assert pie2.getData().asPoint() == [0.5, 0.3, 1.2e-6], "Other aggregation data mismatch"
assert pie2.getLabels() == ["A", "B", "Other"], "Other aggregation labels mismatch"
...
pie3 = ot.Pie(data3)
assert pie3.getData().asPoint() == [0.5, 0.3, 1e-6], "Unlabeled data without aggregation"
# with small value only
data4 = ot.Point([1e-6])
pie4 = ot.Pie(data4, ["tiny"])
assert pie4.getData().asPoint() == [1e-6], "Single small value aggregation"
assert pie4.getLabels() == ["Other"], "Single small value label"
