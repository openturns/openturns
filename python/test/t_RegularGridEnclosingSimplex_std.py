...
for regularGrid in grids:
    ...
    print("regularGrid=", regularGrid, "lowerBound=", lowerBound, "upperBound=", upperBound, n, "simplices")
    algo = ot.RegularGridEnclosingSimplex(regularGrid)

    ot.RandomGenerator.SetSeed(0)
    test = ot.Sample(ot.Uniform(lowerBound - 0.2 * (upperBound - lowerBound), upperBound + 0.2 * (upperBound - lowerBound)).getSample(1000))
    ...
    for vertex in test:
        ...
        if x < lowerBound or x > upperBound:
            ...
            found, coordinates = regularGrid.checkPointInSimplexWithCoordinates(vertex, index)
            if not found:
