...
neighbourIndices_np = map(nearest_debug, test)
...
for x in test:
    if np.any(nearest_debug_indices(x)[:10] != tree.getNearestNeighboursIndices(x, 10, True)):
        print("Errors found in getNearestNeighbours")
        os.exit(1)
