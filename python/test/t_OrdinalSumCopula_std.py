...
print("Survival(inverse survival)=%.5f" % copula.computeSurvivalFunction(InverseSurvival))
# Get 50% quantile
...
if (dim <= 2):
    ...
    print("entropy (MC)=%.5f" % -copula.computeLogPDF(copula.getSample(1000000)).computeMean()[0])
mean = copula.getMean()
print("mean=", mean)
precision = ot.PlatformInfo.GetNumericalPrecision()
ot.PlatformInfo.SetNumericalPrecision(5)
