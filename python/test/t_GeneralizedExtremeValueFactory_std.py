...
ott.assert_almost_equal(estimated_bic_selection.getParameter(), distribution.getParameter(), 5e-2, 5e-2)

default_distribution = factory.build()
print("Default distribution=", default_distribution)
assert default_distribution.getParameter() == ot.GeneralizedExtremeValue().getParameter()
...
assert default_distribution2.getParameter() == ot.GeneralizedExtremeValue().getParameter()
...
assert estimated_param2.getParameter() == distribution.getParameter(), "wrong parameters"
...
ott.assert_almost_equal(estimated_mle.getParameter(), distribution.getParameter(), 1e-2, 1e-2)
...
pdist = ot.ComposedDistribution([ot.Normal(0.0, 2.0),
...
print(estimator)
assert estimator.getParameterDistribution().getImplementation().__class__.__name__ == "Normal"
...
ott.assert_almost_equal(estimated_rmax.getParameter(), [120.707, 12.8155, -0.113177], 1e-2, 1e-2)

estimator_rmax = factory.buildRMaximaEstimator(sample_rmax)
assert estimator_rmax.getParameterDistribution().getImplementation().__class__.__name__ == "Normal"
...
assert estimator_timevar.getParameterDistribution().getImplementation().__class__.__name__ == "Normal"
...
result_deviance = ot.HypothesisTest.LikelihoodRatioTest(estimator_mle.getLogLikelihood(), estimator_timevar.getLogLikelihood())
