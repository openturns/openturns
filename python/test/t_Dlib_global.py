...
rastrigin = ot.SymbolicFunction(['x1', 'x2'], ['20 + x1^2 - 10*cos(2*pi_*x1) + x2^2 - 10*cos(2*pi_*x2)'])

unboundedProblem = ot.OptimizationProblem(rastrigin)

notConstrainingBounds = ot.Interval([-5.0, -5.0], [3.0, 2.0])
notConstrainingBoundsProblem = ot.OptimizationProblem(rastrigin, ot.Function(), ot.Function(), notConstrainingBounds)

constrainingBounds = ot.Interval([-1.0, -2.0], [5.0, -0.5])
constrainingBoundsProblem = ot.OptimizationProblem(rastrigin, ot.Function(), ot.Function(), constrainingBounds)
...
ott.assert_almost_equal(notConstrainingBoundsGlobal.getResult().getOptimalPoint(), unboundedPref, 1, 1e-4)
ott.assert_almost_equal(constrainingBoundsGlobal.getResult().getOptimalPoint(), boundedPref, 1, 1e-4)
