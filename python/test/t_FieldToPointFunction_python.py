...
assert ot.FieldToPointFunction(myFunc) == myFunc
...
class BadFieldToPointFunction(ot.OpenTURNSPythonFieldToPointFunction):
    def __init__(self):
        super().__init__(mesh, 2, 2)
...
with ott.assert_raises(Exception):
    ot.FieldToPointFunction(BadFieldToPointFunction())(ot.Normal(2).getSample(11))


class NonSequenceFieldToPointFunction(ot.OpenTURNSPythonFieldToPointFunction):
    def __init__(self):
        super().__init__(mesh, 2, 2)
...
with ott.assert_raises(Exception):
    ot.FieldToPointFunction(NonSequenceFieldToPointFunction())(
        ot.Normal(2).getSample(11)
    )
