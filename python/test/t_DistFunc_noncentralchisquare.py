    ...
    for i2 in range(n2):
        ...
        for iX in range(nX):
            ...
            print("dNonCentralChiSquare(", nu, ", ", lambda_, ",  %.12g" %
                  x, ")=%.6g" % ot.DistFunc.dNonCentralChiSquare(nu, lambda_, x))
...
for i1 in range(n1):
    ...
    for i2 in range(n2):
        ...
        for iX in range(nX):
            x = xMin + (xMax - xMin) * iX / (nX - 1)
            print("pNonCentralChiSquare(", nu, ", ", lambda_, ",  %.12g" % x, ")=%.6g" % ot.DistFunc.pNonCentralChiSquare(
                nu, lambda_, x), ", complementary=%.6g" % ot.DistFunc.pNonCentralChiSquare(nu, lambda_, x, True))
    ...
    for i2 in range(n2):
        ...
        for iR in range(nR):
            print("rNonCentralChiSquare(", nu, ", ", lambda_, ")=%.6g" %
                  ot.DistFunc.rNonCentralChiSquare(nu, lambda_))
