...
print("simplices volume=", ' '.join(['{:.6f}'.format(mesh1D.computeSimplexVolume(i)) for i in range(mesh1D.getSimplicesNumber())]))
p = [1.3]
print("is p=", p, " in mesh? ", mesh1Ddomain.contains(p))
point = [1.8]
print("Nearest index(", point, ")=", tree.query(point))
nearestIndex = tree.query(point)
found, simplex, coordinates = mesh1D.checkPointInNeighbourhoodWithCoordinates(point, nearestIndex)
...
print("volume=", mesh2D.getVolume())
print("simplices volume=", ' '.join(['{:.6f}'.format(mesh2D.computeSimplexVolume(i)) for i in range(mesh2D.getSimplicesNumber())]))
point = [1.8] * 2
print("Nearest index(", point, ")=", tree.query(point))
...
print("volume=", mesh3D.getVolume())
print("simplices volume=", ' '.join(['{:.6f}'.format(mesh3D.computeSimplexVolume(i)) for i in range(mesh3D.getSimplicesNumber())]))
point = [1.8] * 3
print("Nearest index(", point, ")=", tree.query(point))
