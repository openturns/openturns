...
ott.assert_almost_equal(process.getCompoundProcess().getMesh().getVerticesNumber(), 11)
...
coarseMesh2D = ot.IntervalMesher([5, 5]).build(ot.Interval([0.0, 0.0], [1.0, 1.0]))
gp2D = ot.GaussianProcess(cov2D, coarseMesh2D)
fineMesh2D = ot.IntervalMesher([10, 10]).build(ot.Interval([0.0, 0.0], [1.0, 1.0]))
