...
lbIntegration = ot.Point((0., 0., 0.))
ubIntegration = ot.Point((1., 1., 1.))
routines = ["cuhre", "divonne", "suave", "vegas"]
valueRef = -math.sin(1.) * (math.cos(1.) - 1.) * (math.e - 1.)

for r in routines:
    algoC = ot.CubaIntegration(r)
    value = algoC.integrate(f, ot.Interval(lbIntegration, ubIntegration))
    ott.assert_almost_equal(value[0], valueRef, 1.E-3, 1.E-3)

# Testing three algorithms on a function from R^3 to R^2.
f = ot.SymbolicFunction(["x", "y", "z"], ["exp(-(x * x + y * y + z *z))", "(x / 2 * x / 2 + y / 3 * y / 3 + z / 4 * z / 4 - 1.125)"])
lbIntegration = ot.Point((0., 0., 1.))
ubIntegration = ot.Point((2., 3., 4.))
...
for r in routines:
    ...
    if r == "vegas":
        ...
        algoC.setEpsAbs(5.E-3);
        algoC.setEpsRel(1.E-3);
        algoC.setMaxeval(10000000);
    value = algoC.integrate(f, ot.Interval(lbIntegration, ubIntegration))
    ott.assert_almost_equal(value, valueRef, 1.E-3, 1.E-3)
