...
formula[1] = "sin(" + str(-m.pi) + " + 2 * " + str(m.pi) + " * xi0) + (" + str(a) + ") * (sin(" + str(-m.pi) + " + 2 * " + str(m.pi) + " * xi1)) ^ 2 + (" + \
    str(b) + ") * (" + str(-m.pi) + " + 2 * " + str(
        m.pi) + " * xi2)^4 * sin(" + str(-m.pi) + " + 2 * " + str(m.pi) + " * xi0)"
covTh_Sobol -= 1
# Reference analytical values
meanTh_Ishigami = a / 2.0
covTh_Ishigami = b ** 2 * m.pi ** 8 / 18.0 + \
    (b * m.pi ** 4) / 5.0 + a ** 2 / 8.0 + 1.0 / 2.0
sob_1_Ishigami = ot.Point(3)
sob_1_Ishigami[0] = (
    b * m.pi ** 4 / 5.0 + b ** 2 * m.pi ** 8 / 50.0 + 1.0 / 2.0) / covTh_Ishigami
sob_1_Ishigami[1] = (a ** 2 / 8.0) / covTh_Ishigami
sob_1_Ishigami[2] = 0.0
sob_2_Ishigami = ot.Point(3)
sob_2_Ishigami[0] = 0.0
sob_2_Ishigami[1] = (
    b ** 2 * m.pi ** 8 / 18.0 - b ** 2 * m.pi ** 8 / 50.0) / covTh_Ishigami
