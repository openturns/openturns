...
PDFgrFD = ot.Point(2)
PDFgrFD[0] = (
    ot.Exponential(distribution.getLambda() + eps, distribution.getGamma()).computePDF(
        point
    )
    - ot.Exponential(
        distribution.getLambda() - eps, distribution.getGamma()
    ).computePDF(point)
) / (2.0 * eps)
PDFgrFD[1] = (
    ot.Exponential(distribution.getLambda(), distribution.getGamma() + eps).computePDF(
        point
    )
    - ot.Exponential(
        distribution.getLambda(), distribution.getGamma() - eps
    ).computePDF(point)
) / (2.0 * eps)
...
CDFgrFD = ot.Point(2)
CDFgrFD[0] = (
    ot.Exponential(distribution.getLambda() + eps, distribution.getGamma()).computeCDF(
        point
    )
    - ot.Exponential(
        distribution.getLambda() - eps, distribution.getGamma()
...
CDFgrFD[1] = (
    ot.Exponential(distribution.getLambda(), distribution.getGamma() + eps).computeCDF(
        point
    )
    - ot.Exponential(
        distribution.getLambda(), distribution.getGamma() - eps
