...
meanTh = a / 2
covTh = (b ** 2 * m.pi ** 8) / 18.0 + \
    (b * m.pi ** 4) / 5.0 + (a ** 2) / 8.0 + 1.0 / 2.0
sob_1 = [(b * m.pi ** 4 / 5.0 + b ** 2 * m.pi ** 8 / 50.0 + 1.0 / 2.0)
          / covTh, (a ** 2 / 8.0) / covTh, 0.0]
sob_2 = [
    0.0, (b ** 2 * m.pi ** 8 / 18.0 - b ** 2 * m.pi ** 8 / 50.0) / covTh, 0.0]
sob_3 = [0.0]
...
for adaptiveStrategyIndex in range(len(listAdaptiveStrategy)):
    ...
    for experiment in listExperiment:
        ...
        algo = ot.FunctionalChaosAlgorithm(
            X, Y, distribution, adaptiveStrategy)
        ...
        for i in range(dimension):
            ...
            print("Sobol index", i, "= %.8f" %
                  value, "absolute error=%.10f" % abs(value - sob_1[i]))
        ...
        for i in range(dimension):
            ...
            for j in range(i + 1, dimension):
                ...
                print("Sobol index", indices, "=%.8f" %
                      value, "absolute error=%.10f" % abs(value - sob_2[k]))
                k = k + 1
        indices = ot.Indices([0, 1, 2])
        value = sensitivity.getSobolIndex(indices)
        print("Sobol index", indices, "=%.8f" %
              value, "absolute error=%.10f" % abs(value - sob_3[0]))
        for i in range(dimension):
            value = sensitivity.getSobolTotalIndex(i)
            print("Sobol total index", i, "=%.8f" %
                  value, "absolute error=%.10f" % abs(value - sob_T1[i]))
        indices = ot.Indices(2)
        k = 0
        for i in range(dimension):
            indices[0] = i
        ...
        print("Sobol grouped total index ", indices, "=%.8f" %
              value, "absolute error=%.10f" % abs(value - 1.))
