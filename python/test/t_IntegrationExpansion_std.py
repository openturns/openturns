...
expectedCoefficientsCondensed = [3.5, 1.62542, -0.594723, -1.29064, 1.37242,
                                 -1.95229, 0.194929, -1.08975, 0.409177, 1.35741,
                                 -0.0126684, 0.164588, -0.324901, -0.33939,
                                 0.00046142, -0.0106965, 0.0490707, 0.0459147]
condensedIndices = [0, 1, 7, 10, 15, 30, 35, 40, 49, 77, 84, 89, 98, 156, 165,
                    170, 179, 275]
...
for doe in doeList:
    ...
    if (wMin == wMax):
        # Create the polynomial chaos algorithm using the full constructor
        algo = otexp.IntegrationExpansion(inputSample, outputSample, distribution, productBasis, basisSize)
        algo.run()
        # Check the coefficients
        result = algo.getResult()
        coeffs = result.getCoefficients().asPoint()
