...
for n in range(len(allDistributions)):
    ...
    PDFgrFD = ot.Point(1)
    PDFgrFD[0] = (ot.InverseChiSquare(distribution.getNu() + eps).computePDF(point) -
                  ot.InverseChiSquare(distribution.getNu() - eps).computePDF(point)) / (2.0 * eps)
    print("pdf gradient (FD)=", PDFgrFD)
    CDFgr = distribution.computeCDFGradient(point)
    print("cdf gradient     =", CDFgr)
    CDFgrFD = ot.Point(1)
    CDFgrFD[0] = (ot.InverseChiSquare(distribution.getNu() + eps).computeCDF(point) -
                  ot.InverseChiSquare(distribution.getNu() - eps).computeCDF(point)) / (2.0 * eps)
