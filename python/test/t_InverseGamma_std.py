...
for n in range(len(allDistributions)):
    ...
    PDFgrFD = ot.Point(2)
    PDFgrFD[0] = (ot.InverseGamma(distribution.getLambda() + eps, distribution.getK()).computePDF(point) -
                  ot.InverseGamma(distribution.getLambda() - eps, distribution.getK()).computePDF(point)) / (2.0 * eps)
    PDFgrFD[1] = (ot.InverseGamma(distribution.getLambda(), distribution.getK() + eps).computePDF(point) -
                  ot.InverseGamma(distribution.getLambda(), distribution.getK() - eps).computePDF(point)) / (2.0 * eps)
    print("pdf gradient (FD)=", PDFgrFD)
    CDFgr = distribution.computeCDFGradient(point)
    print("cdf gradient     =", CDFgr)
    CDFgrFD = ot.Point(2)
    CDFgrFD[0] = (ot.InverseGamma(distribution.getLambda() + eps, distribution.getK()).computeCDF(point) -
                  ot.InverseGamma(distribution.getLambda() - eps, distribution.getK()).computeCDF(point)) / (2.0 * eps)
    CDFgrFD[1] = (ot.InverseGamma(distribution.getLambda(), distribution.getK() + eps).computeCDF(point) -
                  ot.InverseGamma(distribution.getLambda(), distribution.getK() - eps).computeCDF(point)) / (2.0 * eps)
