...
distribution = ot.ComposedDistribution(aCollection, ot.IndependentCopula(dim))
# Test for sampling
size = 10000
...
print("inverse transform gradient at transformed point (FD)=", repr(ot.CenteredFiniteDifferenceGradient(
...
left = ot.ComposedDistribution(coll).getIsoProbabilisticTransformation()
coll[0] = ot.Uniform(-1.0 - eps, 2.0)
right = ot.ComposedDistribution(coll).getIsoProbabilisticTransformation()
...
left = ot.ComposedDistribution(
    coll).getInverseIsoProbabilisticTransformation()
coll[0] = ot.Uniform(-1.0 - eps, 2.0)
