...
>>> for j in range(grid.getNbColumns()):
...    beta = 1.0 + j
...    grid.setGraph(0, j, ot.Gumbel(beta, 0.0).drawCDF())
>>> grid.setLegendCorner([0.5, 0.5]) # legend will begin in the middle
...
R"RAW(Accessor to the collection of graphs.

Parameters
----------
coll : sequence of :class:`~openturns.Graph`
    The graphs to store into the :class:`~openturns.GridLayout`. The collection must have at most :math:`nbRows\times nbColumns` elements.)RAW"
