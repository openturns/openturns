...
R"RAW(Least squares metamodel selection factory.
...
- *LeastSquaresMetaModelSelection-MaximumErrorFactor*: through the exploration, the minimum error :math:`e_{min}` is stored. If the
  ...
  The default value is 2. This value allows one to filter little fluctuations in the error computation.
