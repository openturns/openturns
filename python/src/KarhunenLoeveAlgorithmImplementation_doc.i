...
R"RAW(Base class for Karhunen-Loeve algorithms.
...
R"RAW(Accessor to the threshold used to select the most significant eigenmodes.
...
Notes
-----
OpenTURNS truncates the sequence :math:`(\lambda_k,  \vect{\varphi}_k)_{k \geq 1}`  at the index :math:`K` defined in :eq:`thresholdK`.)RAW"
%enddef
...
----------
s : float, :math:`\geq 0`
    The threshold :math:`s` defined in :eq:`thresholdK`.)RAW"
%enddef
