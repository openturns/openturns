...
R"RAW(L2 approximation on an orthonormal basis using integration strategy and a fixed basis.
...
in the class :class:`~openturns.FunctionalChaosAlgorithm`.

This class enables to estimate a functional chaos expansion defined in
:any:`functional_chaos` by equation :eq:`metaModelPn` with no model selection: the model
is approximated in the specified functional space generated by the specified *basis*, and the
coefficients :math:`(a_k)_{k \in I_n}` are computed using the inner product defined by the
specified *distribution*.

This computation makes sense if and only if the specified *basis* is orthonormal with respect
to the *distribution*. When the *basis* is not specified, it is built as the
tensorization of
the univariate orthonormal polynomial bases associated to each marginal
of *distribution*: it implies that the *basis* is orthogonal to the input distribution only in
