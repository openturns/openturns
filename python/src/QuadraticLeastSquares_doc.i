...
Refer to :ref:`least_squares` to get details on general least squares surrogate models and
to get information on the estimation of
the symmetric tensor :math:`\tens{M}`,
...
Notes
-----
The center vector :math:`\vect{b}` is defined in :eq:`QuadraticLeastSquaresMMOpenTURNSAPI`.
)RAW"
...
Notes
-----
To get the surrogate model :math:`\metaModel` defined in :eq:`QuadraticLeastSquaresMMOpenTURNSAPI` as a :class:`~openturns.Function`, use the :meth:`~openturns.MetaModelResult.getMetaModel` method.
)RAW"
