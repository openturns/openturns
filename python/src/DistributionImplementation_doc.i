...
R"RAW(Compute the characteristic function.
...
R"RAW(Compute the complementary cumulative distribution function.
...
R"RAW(Compute the conditional cumulative distribution function.
...
For :math:`j=1`, it
reduces to :math:`F_{X_1}(x_1)`.)RAW"
%enddef
...
R"RAW(Compute the conditional derivative density function of the last component.
...
See Also
--------
computeDDF, computeConditionalCDF)RAW"
%enddef
...
R"RAW(Compute the conditional probability density function.
...
See Also
--------
computePDF, computeConditionalCDF)RAW"
%enddef
...
R"RAW(Compute the conditional quantile function of the last component.
...
See Also
--------
computeQuantile, computeConditionalCDF)RAW"
%enddef
...
R"RAW(Compute the sequential conditional cumulative distribution functions.
...
R"RAW(Compute the sequential conditional derivative density function.
...
The first term, for :math:`j=1`, is :math:`\dfrac{d^2}{d\,x_1^2}F_{X_1}(x_1)`.)RAW"
%enddef
...
R"RAW(Compute the sequential conditional probability density function.
...
R"RAW(Compute the derivative density function.
...
R"RAW(Compute the entropy of the distribution.
...
R"RAW(Compute the probability-generating function.
...
R"RAW(Compute the logarithm of the probability density function.
...
R"RAW(Compute the gradient of the log probability density function.
...
R"RAW(Compute the probability density function.
...
:math:`\vect{c}` such that :math:`c_i = a_i`.)RAW"
%enddef
...
R"RAW(Compute the quantile function.
...
R"RAW(Compute the cumulative distribution function of the squared radius.
...
.. math::
    ...
    R^2 = \sqrt{\sum\limits_{i=1}^n U_i^2})RAW"
%enddef
...
R"RAW(Compute the quantile function for univariate distributions.
...
R"RAW(Compute the survival function.
...
R"RAW(Compute the inverse survival function.
...
See Also
--------
computeQuantile, computeSurvivalFunction)RAW"
%enddef
...
R"RAW(Draw the cumulative distribution function of a margin.
...
R"RAW(Compute the confidence domain with minimum volume.
...
R"RAW(Compute the confidence interval with minimum volume.
...
R"RAW(Compute a bilateral confidence interval.
...
R"RAW(Compute a unilateral confidence interval.
...
R"RAW(Draw the probability density function of a margin.
...
R"RAW(Draw the log-probability density function of a margin.
...
R"RAW(Draw the cumulative distribution function of a couple of margins.
...
R"RAW(Draw the probability density function of a couple of margins.
...
R"RAW(Draw the log-probability density function of a couple of margins.
...
R"RAW(Accessor to the componentwise central moments.
...
R"RAW(Accessor to the componentwise shifted moments.
...
See Also
--------
getMoment, getCentralMoment)RAW"
%enddef
...
R"RAW( Accessor to the correlation matrix.
...
.. math::
    ...
    \mat{\Sigma} & = \Cov{\vect{X}} \\
                 & = \Expect{\left(\vect{X} - \vect{\mu}\right)
                             \Tr{\left(\vect{X} - \vect{\mu}\right)}})RAW"
%enddef
...
R"RAW(Accessor to the inverse iso-probabilistic transformation.
...
See also
--------
openturns.Distribution.getIsoProbabilisticTransformation)RAW"
%enddef
...
openturns.Distribution.hasEllipticalCopula)RAW"
%enddef
...
R"RAW(Accessor to the Kendall coefficients matrix.
...
R"RAW(Accessor to the componentwise kurtosis.
...
R"RAW(Accessor to marginal distributions.
...
distribution : :class:`~openturns.Distribution`
    The marginal distribution of the selected component(s).)RAW"
%enddef
...
.. math::
    ...
    \vect{\mu} = \Tr{\left(\Expect{X_i}, \quad i = 1, \ldots, n\right)})RAW"
%enddef
...
R"RAW(Accessor to the componentwise moments.
...
.. math::
    ...
    \vect{m}^{(k)} = \Tr{\left(\Expect{X_i^k}, \quad i = 1, \ldots, d\right)})RAW"
%enddef
...
R"RAW(Accessor to the Pearson correlation matrix.
...
R"RAW(Accessor to roughness of the distribution.
...
%define OT_Distribution_getPDF_doc
...
%enddef
%feature("docstring") OT::DistributionImplementation::getPDF
OT_Distribution_getPDF_doc
// ---------------------------------------------------------------------

%define OT_Distribution_getLogPDF_doc
...
%enddef
%feature("docstring") OT::DistributionImplementation::getLogPDF
OT_Distribution_getLogPDF_doc

// ---------------------------------------------------------------------

%define OT_Distribution_getCDF_doc
...
%enddef
%feature("docstring") OT::DistributionImplementation::getCDF
...
R"RAW(Accessor to the componentwise skewness.
...
R"RAW(Accessor to the Spearman correlation matrix.
...
generic algorithms of orthonormalization (see :class:`~openturns.experimental.StandardDistributionPolynomialFactory`)."
%enddef
...
R"RAW(Accessor to the support of the distribution.
...
R"RAW(Test whether the distribution is elliptical or not.
...
*characteristic generator* of the elliptical distribution.)RAW"
%enddef
...
R"RAW(Compute the upper tail dependence coefficients.
...
>>> chi = copula.computeUpperTailDependenceMatrix()[1, 0])RAW"
%enddef
...
R"RAW(Compute the upper extremal dependence coefficients.
...
>>> import openturns as ot
>>> copula = ot.FrankCopula()
>>> chib = copula.computeUpperExtremalDependenceMatrix()[1, 0])RAW"
%enddef
...
R"RAW(Compute the lower tail dependence coefficients.
...
>>> import openturns as ot
>>> copula = ot.FrankCopula()
>>> chiL = copula.computeLowerTailDependenceMatrix()[1, 0])RAW"
%enddef
...
R"RAW(Compute the lower extremal dependence coefficients.
...
>>> import openturns as ot
>>> copula = ot.FrankCopula()
>>> chiLb = copula.computeLowerExtremalDependenceMatrix()[1, 0])RAW"
%enddef
...
%define OT_Distribution_getSupportEpsilon_doc
...
%enddef
%feature("docstring") OT::DistributionImplementation::getSupportEpsilon
OT_Distribution_getSupportEpsilon_doc

// ---------------------------------------------------------------------

%define OT_Distribution_setSupportEpsilon_doc
"Accessor to the support tolerance for discrete distributions.
...
epsilon : float
    The support tolerance."
%enddef
%feature("docstring") OT::DistributionImplementation::setSupportEpsilon
OT_Distribution_setSupportEpsilon_doc

// ---------------------------------------------------------------------

%define OT_Distribution_isParallel_doc
