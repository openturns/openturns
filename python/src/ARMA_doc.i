...
R"RAW(Get possible futures from the current state of the ARMA process.
...
Note that the time grid of each future begins at the last time stamp of the time grid associated to the time series which is extended.)RAW"
...
R"RAW(Accessor to the white noise defining the ARMA process.

Returns
-------
whiteNoise : :class:`~openturns.WhiteNoise`
    The white noise :math:`\varepsilon` used in the linear recurrence of the ARMA process.)RAW"
...
R"RAW(Accessor to the stored state of the ARMA process.
...
R"RAW(Accessor to the number of time stamps used to thermalize the process.
...
Nther : int, :math:`N_{ther} \geq 1`
    ...
    Default precision is: :math:`\varepsilon = 2^{-53} \equiv 10^{-16}`.)RAW"
...
----------
Nther : int, :math:`N_{ther} \geq 1`
    The number of time stamps used to make the ARMA realization independent from its actual state.)RAW"
