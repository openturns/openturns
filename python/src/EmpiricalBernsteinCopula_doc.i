...
R"RAW(Empirical Bernstein copula.
...
Care: the EmpiricalBernsteinCopula may not be a copula. It is only a copula if :math:`m` divides :math:`\sampleSize`. In the other case, it is a core,
i.e a multivariate distribution whose range is included in :math:`[0,1]^d`.

**Case 1:** :math:`isRankSample = False`.

In that case, the given sample is not assumed to be already a normalized rank sample, ie the sample generated by a copula.
If :math:`m` does not divide :math:`\sampleSize`, then a part of the sample is
...
:math:`s_i^j`. We denote by :math:`F_{B(r_i^j, s_i^j, 0, 1)}` its cumulative distribution function.
...
R"RAW(Get the bin number :math:`m`.

Returns
-------
m : int, :math:`0 < m \leq \sampleSize`
    The number of sub-intervals in which all the edges of the unit cube
    :math:`[0, 1]^d` are regularly partitioned.)RAW"

// ---------------------------------------------------------------------

%feature("docstring") OT::EmpiricalBernsteinCopula::setBinNumber
R"RAW(Set the bin number :math:`m`.
