...
- ``PosteriorDistribution-OptimizationAlgorithm`` (``String``, default: ``Cobyla``): name of the optimization algorithm used to compute the maximum of the log-posterior density of the ratio-of-uniforms sampler. The possible values are ``AbdoRackwitz``, ``Cobyla``, ``SQP`` and ``TNC``, plus the names returned by the method :meth:`OptimizationAlgorithm.GetAlgorithmNames`.
- ``PosteriorDistribution-RatioUniformCandidateNumber`` (``UnsignedInteger``, default: ``10000``): number of candidate points generated by the ratio-of-uniforms sampler
