...
- the key `BernsteinCopulaFactory-BinNumberSelectionMethod` (default: 'LogLikelihood') that defines the criterion to compute the optimal bin number
  when it is not specified. The possible choices are 'AMISE', 'LogLikelihood', 'PenalizedCsiszarDivergence';
- the key `BernsteinCopulaFactory-kFraction` (default: 2) that defines the fraction of the sample used for the validation in the
  method :meth:`ComputeLogLikelihoodBinNumber`,
- the key `BernsteinCopulaFactory-alpha` (default: 1.0) that defines the weight of the penalization term used in the
