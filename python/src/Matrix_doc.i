...
- *Matrix-size-visible-in-str-from* (`UnsignedInteger`, default: `5`)
)RAW"
...
R"RAW(Compute the QR factorization.
...
R"RAW(Compute the singular values.
...
R"RAW(Compute the singular values decomposition (SVD).
...
R"RAW(Reshape the matrix, in place.
...
R"RAW(Compute the associated Gram matrix.
...
R"RAW(Compute the Hadamard product matrix.
...
R"RAW(Compute the sum of the matrix elements.
