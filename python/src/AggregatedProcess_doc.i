...
R"RAW(Aggregation of several processes in one process.
...
R"RAW(Get one realization of the aggregated process.

Returns
-------
realization : :class:`~openturns.Field`
    Each process of the collection is realized on the common mesh defined on :math:`\cD_0`.)RAW"
...
realization : :class:`~openturns.Function`
    Each process of the collection is continuously realized on the common domain :math:`\cD_0`.)RAW"
