...
R"RAW(Accessor to the importance factors.
...
R"RAW(Accessor to the Hasofer Reliability Index.

Returns
-------
index : float
    Hasofer Reliability Index which is the distance of the design point from
    the origin of the standard space :math:`\beta_{HL}=||\vect{u}^*||`.)RAW"
...
R"RAW(Accessor to the mean point in the standard event domain.
