...
\begin{itemize}
    \item \textbf{Eigen} est déjà dépendance OpenTURNS ; utiliser pour toute l'algèbre linéaire espaces tangents.
...
\midrule
Classe abstraite \texttt{Manifold} & Interface: dimension, exp/log map, métrique, volume, projection pushforward/pullback & Eigen (existant) & 1.5 ps \\
Implémentation $\mathcal{S}^{n-1}$ & Sphère: exp/log analytiques, métrique induite, volume élément & Eigen & 0.5 ps \\
...
ACP Tangente & Projection données sur $T_{\mu_F}\mathcal{M}$, ACP standard, re-projection & Eigen, Geomstats & 1 ps \\
\bottomrule
\end{longtable}
...
\begin{itemize}
    ...
    \item Dépendances externes minimales : Eigen (existant), Manif/Sophus (header-only), Boost.Math (existant), pas de nouvelle dépendance d'optimisation riemannienne (implémentation native gradient/rétraction).
\end{itemize}
