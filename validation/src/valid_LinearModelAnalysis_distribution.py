...
class SampleEstimatorLinearRegression(ot.PythonRandomVector):
    def __init__(self, sample_size, standard_deviation, coefficients, estimator, x0=None):
        ...
        if self.estimator in ["prediction", "observation"] and x0 is None:
            raise ValueError(f"The 'x0' parameter cannot be None when estimator is '{self.estimator}'.")
        self.x0 = x0
    ...
    def getRealization(self):
        result = None
        regressionResult = self.getRegressionResult()
        if self.estimator == "variance":
            result = [regressionResult.getResidualsVariance()]
        elif self.estimator.startswith("coefficient_"):
            index = int(self.estimator.split("_")[-1])
            result = [regressionResult.getCoefficients()[index]]
        elif self.estimator == "prediction":
            metamodel = regressionResult.getMetaModel()
            result = metamodel(self.x0)
        elif self.estimator == "observation":
            metamodel = regressionResult.getMetaModel()
            error = self.error_distribution.getRealization()
            result = metamodel(self.x0) + error
        else:
            raise ValueError("Unknown estimator: " + self.estimator)
        return result
